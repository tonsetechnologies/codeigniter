...
// Amended to check (another!) div by 0 and provide a proper initialisation function: patch by justhatched.
// End changes - OscarB
...
class Chart{
    ...
    // OscarB - Add a proper initialisation function: patch by justhatched
    //          Without it, plotting multiple graphs on one page will just concatenate the series.
	...
	function plot($file){  // patch this code changed by Emilio Veloso
        ...
        // trap div by zero, patch by justhatched
		$h = !$maxvalues ? 1 : ($height / $maxvalues);
		$dy = !($max-$min) ? 1 : $height / ($max-$min);
		$vdy = !$maxvalues ? 1 : ($max-$min) / $maxvalues;
