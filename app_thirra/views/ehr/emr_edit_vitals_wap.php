...
echo "\n<input type='text' name='reading_date' value='".set_value('reading_date',$init_reading_date)."' size='8' /> YYYY-MM-DD</td></tr>";
...
?>
    <script  type="text/javascript">
      ...
      {
         if ($("waistCircumference").value == '')
            $("wcInch").value = '';
         else
         {
            var waistCircumference = Number($("waistCircumference").value);
    ...
    window.onload = function() {
        $("height").onChange = calculateHeightInch;
        $("height").onChange = calculateBMI;
        $("weight").onChange = calculateBMI;
        $("waistCircumference").onChange = calculateWCInch;
    }
  </script>
