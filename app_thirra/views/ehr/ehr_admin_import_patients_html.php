...
if(isset($fileList)){
    ...
    foreach ($fileList as $unsynched_item){
	    if (is_file("$directory/$unsynched_item") && $unsynched_item != '.' && $unsynched_item != '..' && (substr($unsynched_item,0,12)=="patient_demo")) {
            ...
            echo "\n<td> &nbsp; ".anchor('ehr_admin_edi_import/admin_import_new_patients/'.$unsynched_item, 'Import')."</td>";
            echo "\n</tr>";
            $rownum++;
	    }
    }//endforeach;
...
echo form_open('ehr_admin_edi/admin_upload_new_patients', $attributes);
...
if(isset($synched_list)){
    ...
    foreach ($synched_list as $synched_item){
        echo "\n<tr>";
        echo "\n<td>".$rownum.".</td>";
        echo "\n<td>".anchor('ehr_admin_edi/individual_overview/'.$synched_item['patient_id'], "<strong>".$synched_item['name']."</strong>, ".$synched_item['name_first'], 'target="_blank"')."</td>";
