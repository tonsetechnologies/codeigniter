...
echo anchor('ehr_orders/orders_edit_lab_package/edit_package/'.$package_info[0]['supplier_id'].'/'.$package_info[0]['lab_package_id'],  $package_info[0]['package_name'])."</td><tr>";

echo "\n<tr><td>Lab Package Code</td><td>";
echo $package_info[0]['package_code']."</td><tr>";

echo "\n<tr><td>Lab Supplier</td><td><strong>";
echo $package_info[0]['supplier_name']."</strong></td><tr>";
...
echo "\n<tr>";
    echo "\n<td width='25%'>LOINC Class </td>";
    echo "<td colspan='2' class='red'>";   
    echo $loinc_class_name;
    /*
    ...
    echo "\n<td colspan='2' class='red'>";
        echo "\n<select name='loinc_num' class='normal' onchange='javascript:select_LOINC()' id='loinc_num'>";
            ...
            foreach($loinc_list as $loinc_item)
            {
                ...
                echo ">".$loinc_item['component']." &nbsp;&nbsp;[".$loinc_item['loinc_num']."] ".$loinc_item['system']."</option>";
            } //foreach
        echo "</select>";
    echo "</td>";
echo "</tr>";
...
if($num_of_tests > 0){
    ...
    foreach ($tests_list as $test_item){
        ...
        echo "\n<tr>";
        echo "<td>".$test_item['sort_test']."</td>";
...
?>
    <script  type="text/javascript">
        ...
        function select_loinc_class(){
            document.getElementById("packagetest_form").status.value="Unfinished";
            document.getElementById("loinc_num").selectedIndex = -1;
            document.getElementById("one_package").selectedIndex = -1;
            document.getElementById("one_holder").selectedIndex = -1;
            document.getElementById("packagetest_form").submit.click();
        }
