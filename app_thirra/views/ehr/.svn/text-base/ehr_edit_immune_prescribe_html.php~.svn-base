...
foreach ($prescribe_list as $prescribe_item){
    echo "\n<tr>";
    echo "<td>".anchor('ehr_consult/edit_immune_prescribe/edit_prescribe/'.$patient_id.'/'.$summary_id.'/'.$prescribe_item['queue_id'], $prescribe_item['generic_name'])."</td>";
...
echo form_open('ehr_consult/edit_immune_prescribe/new_immune/'.$patient_id, $attributes);
...
?>
    <script  type="text/javascript">
        ...
        function selectDrugSystem(){
            $("prescription_form").status.value="Unfinished";
            $("drug_formulary_id").selectedIndex = -1;
            $("tradename").selectedIndex = -1;
            $("drug_batch").selectedIndex = -1;
            $("prescription_form").submit.click();
        }
