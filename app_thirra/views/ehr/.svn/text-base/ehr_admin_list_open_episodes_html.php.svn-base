...
if(isset($unsynched_list)){
    ...
    foreach ($unsynched_list as $unsynched_item){
        ...
        echo anchor('ehr_consult/consult_episode/'.$unsynched_item['patient_id'].'/'.$unsynched_item['summary_id'], $unsynched_item['date_started']." ".$unsynched_item['time_started'], 'target="_blank"');
        echo "\n</td>";
