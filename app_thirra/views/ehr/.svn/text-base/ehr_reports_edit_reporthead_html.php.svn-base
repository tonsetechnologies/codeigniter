...
echo "\n<p>".anchor('ehr_reports/reports_select_report/edit_param/'.$report_header_id, ' <strong>PRINT REPORT</strong>', "target='_blank'")."</p>";

//echo validation_errors(); Displays ALL errors in one place.
echo form_open('ehr_reports/reports_edit_reporthead/'.$form_purpose.'/'.$report_header_id);
...
if($report_header_id <> 'new_report'){
    echo anchor('ehr_reports/reports_edit_reportbody/new_body/'.$report_header_id.'/new_body', "<strong>Add Column</strong>");
} else {
    echo "Add Column";
}
echo "\n<br /><br />";
    ...
    $delete_link    =   base_url()."index.php/ehr_reports/reports_delete_reportbody/".$body_item['report_header_id']."/".$body_item['report_body_id'];
