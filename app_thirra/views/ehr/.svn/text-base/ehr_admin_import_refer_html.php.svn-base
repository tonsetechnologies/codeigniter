...
echo "\n\n<div id='page_title' align='center'><h1>".$this->lang->line('admin_import_refer_html_title')."</h1></div>";
...
if(isset($fileList)){
    ...
    foreach ($fileList as $unsynched_item){
	    if (is_file("$directory/$unsynched_item") && $unsynched_item != '.' && $unsynched_item != '..' && (substr($unsynched_item,0,13)=="patient_refer")) {
		    echo "\n<tr>";
            echo "\n<td>".$rownum.".</td>";
            //echo "\n<td><a href='/thirra-uploads/imports_refer/".$unsynched_item."' target='_blank'>".$unsynched_item."</td>";
            echo "\n<td><a href='http://".$_SERVER["SERVER_ADDR"]."/".$app_folder."-uploads/imports_refer/".$unsynched_item."' target='_blank'>$unsynched_item</a></td>";
            echo "\n<td align='right'>".filesize("$directory/$unsynched_item")."</td>";
            echo "\n<td> &nbsp; ".anchor('ehr_admin_edi/admin_import_new_refer/'.$unsynched_item, 'Import', 'target="_blank"')."</td>";
            echo "\n</tr>";
            $rownum++;
	    }
    }//endforeach;
