...
if(count($complaints_list) > 0){
    foreach ($complaints_list as $complaint_item){
        ...
        $delete_link    =   base_url()."index.php/ehr_consult/consult_delete_complaint/".$patient_id."/".$summary_id."/".$complaint_item['complaint_id'];
        echo "\n<td><a href=\"javascript:deleteRecord('remove','Complaint:".$complaint_item['full_description']."','".$delete_link ."');\">delete</a>";
        //echo "\n<td>".anchor('ehr_consult/consult_delete_complaint/'.$patient_id.'/'.$summary_id.'/'.$complaint_item['complaint_id'], 'delete')."</td>";
        echo "</tr>";
    }//endforeach;
} else {
...
echo form_open('ehr_consult/edit_reason_encounter/'.$patient_id, $attributes);
    ...
    echo "\n<td colspan='2' class='red'>";
        echo "\n<select name='complaintCode' class='normal' onchange='javascript:selectLevel2()' id='complaintCode'>";
            ...
            foreach($ccode1_list as $ccode1)
            {
                ...
                echo ">[".$ccode1['icpc_code']."] &nbsp;&nbsp;&nbsp;".$ccode1['full_description']."</option>";
            } //foreach
        echo "</select>";
    echo "</td>";
echo "</tr>";
