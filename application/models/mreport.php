...
class MReport extends CI_Model
{
    ...
    {
        $dataset = array();
        $sqlQuery   =   "SELECT pdem.patient_id, clinic_reference_number, pns_pat_id, pdem.nhfa_no, name, name_first, gender, 
    ...
    function get_rpt_immunisation_done($expect='data',$sort_order='pdem.name',$rows=NULL,$param=NULL)
    {
        ...
        if ($query->num_rows() > 0){
            ...
            foreach ($query->result_array() as $row)
            {
                ...
                $dataset[$rownum][18]['val']  = $row['tel_mobile'];
                $dataset[$rownum][18]['desc']  = "tel_mobile";
                $dataset[$rownum][19]['field']  = "vaccined_date";
                $dataset[$rownum][19]['val']  = $row['vaccined_date'];
                $dataset[$rownum][19]['desc']  = "vaccined_date";
                $dataset[$rownum][20]['field']  = "vaccine";
                $dataset[$rownum][20]['val']  = $row['vaccine'];
                $dataset[$rownum][20]['desc']  = "vaccine";
                $dataset[$rownum][21]['field']  = "dose";
                $dataset[$rownum][21]['val']  = $row['dose'];
                $dataset[$rownum][21]['desc']  = "dose";
                $dataset[$rownum][22]['field']  = "immunisation_sort";
                $dataset[$rownum][22]['val']  = $row['immunisation_sort'];
                $dataset[$rownum][22]['desc']  = "immunisation_sort";
                $dataset[$rownum][23]['field']  = "clinic_name";
                $dataset[$rownum][23]['val']  = $row['clinic_name'];
                $dataset[$rownum][23]['desc']  = "clinic_name";
                $dataset[$rownum][24]['field']  = "addr_village_name";
                $dataset[$rownum][24]['val']  = $row['addr_village_name'];
                $dataset[$rownum][24]['desc']  = "addr_village_name";
                $dataset[$rownum][25]['field']  = "addr_town_name";
                $dataset[$rownum][25]['val']  = $row['addr_town_name'];
                $dataset[$rownum][25]['desc']  = "addr_town_name";
                $rownum++;            
            }
        }    
        if($expect == 'data'){
