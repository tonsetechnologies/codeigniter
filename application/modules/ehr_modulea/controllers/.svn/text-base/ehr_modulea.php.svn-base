 ...
 */
class Ehr_modulea extends MX_Controller 
{
    ...
    // === QUEUE MANAGEMENT
    // ------------------------------------------------------------------------
    function modulea_mgt($id=NULL)  // template for new classes
    {
    ...
    // ------------------------------------------------------------------------
    function queue_edit_queue($form_purpose,$patient_id,$booking_id)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		...
		$data['form_purpose']   = $form_purpose;//$this->uri->segment(9);
        $data['patient_id']     = $patient_id;//$this->uri->segment(10);
        $data['booking_id']     = $booking_id;//$this->uri->segment(11);
        $data['now_id']             =   time();
        $data['now_date']           =   date("Y-m-d",$data['now_id']);
        $data['now_time']           =   date("H:i",$data['now_id']);
        ...
        if(count($_POST)) {
            ...
            */
            ...
            $data['booking_id']         = $booking_id;//$this->uri->segment(5);
		...
		if ($this->form_validation->run('edit_queue') == FALSE){
            ...
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_banner =   "ehr/banner_ehr_html";
                $new_sidebar=   "ehr/sidebar_emr_queue_html";
                $new_body   =   "ehr_queue_edit_queue_html";
                $new_footer =   "ehr/footer_emr_html";
            }
            if($data['user_rights']['section_queue'] < 100){
                $new_body   =   "ehr/ehr_access_denied_html";
            ...
            $new_page = base_url()."index.php/ehr/ehr/index/ehr_queue/ehr_queue/queue_mgt";
            header("Status: 200");
            header("Location: ".$new_page);
    ...
    // ------------------------------------------------------------------------
    function queue_edit_room($seg3,$seg4) 
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$this->load->model('madmin_rdb');
		...
		$data['room_id'] 	        = $seg4; //$this->uri->segment(4);
        $data['location_id']        =   $this->session->userdata('location_id');
		...
		if ($this->form_validation->run('edit_room') == FALSE){
            ...
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_banner =   "ehr/banner_ehr_wap";
                $new_sidebar=   "ehr/sidebar_ehr_queue_wap";
                $new_body   =   "ehr_queue_edit_room_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
