...
if($debug_mode) {
    ...
    echo "\n<br />pcode1set=".$init_pcode1set;
    echo "\n<br />pcode1_code=".$init_pcode1_code;
    echo "\n</div>";
}

echo "\n\n<div id='page_title' align='center'>\n<h1>".$this->lang->line('util_edit_procedure_group_html_title')."</h1></div>\n";


$attributes =   array('class' => 'select_form', 'id' => 'consultation_form');
echo form_open('ehr/ehr/index/ehr_utilities/ehr_utilities_codes_procedures/util_edit_procedure_group/'.$form_purpose.'/'.$init_pcode1_id, $attributes);

echo form_hidden('form_purpose', $form_purpose);
echo form_hidden('now_id', $now_id);
echo form_hidden('pcode1_id', $pcode1_id);
...
echo form_error('pcode1_code');
echo "\n<input type='text' name='pcode1_code' value='".set_value('pcode1_code',$init_pcode1_code)."' size='10' /></td></tr>";
...
echo "<tr>";
    echo "\n<td width='25%'>Code Category <font color='red'>*</font></td>";
    echo "<td colspan='2' class='red'>";   
    echo form_error('pcode_category');
    echo "\n<div id='diagnosisChapter_select'>";
    echo "\n<select name='pcode_category' class='normal' id='diagnosisChapter'>";
        echo "\n<option label='' value='' selected='selected'>Please select one</option>";
        foreach($pcode_categories as $category)
        {
            echo "\n<option label='".$category['pcode_category']."' value='".$category['pcode_category']."'";
            echo ($init_pcode_category == $category['pcode_category'] ? " selected='selected'" : "");
            echo ">".$category['pcode_category']."</option>";
        }
        echo "<option label='Others' value='Others' ";
        echo ($init_pcode_category == 'Others' ? " selected='selected'" : "");
        echo ">Others</option>";
        echo "</select>";
    echo "</div>";
    echo "</td>";
...
echo "\n<tr><td>Active</td><td>";
echo form_error('pcode1_active');
//echo "<input type='text' name='pcode1ext_active' value='".set_value('pcode1ext_active',$init_pcode1ext_active)."' size='5' /></td></tr>";
echo $init_pcode1_active."</td></tr>";
echo "\n<input type='hidden' name='pcode1_active' value='".$init_pcode1_active."' />";
