 ...
 */
class Ehr_pharmacy extends MX_Controller 
{
    ...
    function pharmacy_mgt($id=NULL)  // template for new classes
    {
        ...
        $data['dispensing_info']  = $this->mpharma_rdb->get_pending_dispensings('Pending',$data['location_id']);
        $data['prescription_info']  = $this->mpharma_rdb->get_pending_prescriptions('Pending',$data['location_id']);
        $this->load->vars($data);
		if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_body   =   "ehr_pharmacy_mgt_html";
    ...
    // ------------------------------------------------------------------------
    function print_prescription($seg3,$seg4,$seg5,$seg6,$seg7)  // Print prescription to HTML or PDF
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
		...
		$data['now_id']         =   time();
		$data['hide_banner'] 	= $seg3; //$this->uri->segment(3);
		$data['output_format'] 	= $seg4; //$this->uri->segment(3);
		$data['form_purpose'] 	= $seg5; //$this->uri->segment(3);
		$data['patient_id'] 	= $seg6; //$this->uri->segment(3);
		$data['summary_id'] 	= $seg7; //$this->uri->segment(4);
        if($data['form_purpose'] == "print_dispensing"){
            $data['prescribe_list'] = $this->memr_rdb->get_patcon_dispense($data['summary_id']);
        } else {
		...
		$broken_age				=	break_date($data['patient_info']['birth_date']);
		...
		if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_sidebar=   "ehr/sidebar_ehr_pharmacy_wap";
            $new_body   =   "ehr_print_prescription_html";
            $new_footer =   "ehr/footer_emr_wap";
		} else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
    ...
    // ------------------------------------------------------------------------
    function phar_close_prescription($seg3,$seg4) 
    {
        ...
        $this->session->set_flashdata('data_activity', 'Prescription archived.');
		
        $new_page = base_url()."index.php/ehr/ehr/index/ehr_pharmacy/ehr_pharmacy/pharmacy_mgt";
        header("Status: 200");
        header("Location: ".$new_page);
    ...
    function phar_listclosed_prescriptions($id=NULL)  // List suppliers for labs
    {
		...
		if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_body   =   "ehr_phar_listclosed_prescriptions_html";
            $new_footer =   "ehr/footer_emr_wap";
		} else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
    ...
    function phar_list_drug_packages($id=NULL)  // List drug packages
    {
        ...
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
        $data['breadcrumbs']        =   breadcrumbs('ehr/ehr/index/ehr_pharmacy/ehr_pharmacy/pharmacy_mgt','Pharmacy');    
		...
		if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_body   =   "ehr_phar_list_drug_packages_html";
            $new_footer =   "ehr/footer_emr_wap";
		} else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
    ...
    function phar_edit_drug_package($seg3,$seg4)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
        $data['breadcrumbs']        =   breadcrumbs('ehr/ehr/index/ehr_pharmacy/ehr_pharmacy/pharmacy_mgt','Pharmacy','ehr_pharmacy/phar_list_drug_packages','List Drug Packages');    
		$data['form_purpose'] 		= $seg3; //$this->uri->segment(3);
		$data['drug_package_id'] 	= $seg4; //$this->uri->segment(4);
		...
		if ($this->form_validation->run('edit_drug_package') == FALSE){
            ...
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_body   =   "ehr_phar_edit_drug_package_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
            ...
            $new_page = base_url()."index.php/ehr/ehr/index/ehr_pharmacy/ehr_pharmacy/phar_list_drug_packages";
            header("Status: 200");
            header("Location: ".$new_page);
    ...
    function phar_edit_package_drug($seg3,$seg4,$seg5)
    {
		...
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
		$data['form_purpose'] 		= $seg3; //$this->uri->segment(3);
		$data['content_id'] 		= $seg4; //$this->uri->segment(4);
		$data['drug_package_id'] 	= $seg5; //$this->uri->segment(5);
		...
		if ($this->form_validation->run('edit_prescribe') == FALSE){
            ...
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_sidebar=   "ehr/sidebar_emr_admin_wap";
                //$new_body   =   "ehr/emr_admin_edit_referral_person_wap";
                $new_body   =   "ehr_phar_edit_package_drug_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
            ...
            $new_page = base_url()."index.php/ehr/ehr/index/ehr_pharmacy/ehr_pharmacy/phar_edit_drug_package/edit_package/".$data['drug_package_id'];
            header("Status: 200");
            header("Location: ".$new_page);
    ...
    // ------------------------------------------------------------------------
    function phar_delete_package_drug($seg3,$seg4) 
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
		$data['drug_package_id'] 	= $seg3; //$this->uri->segment(3);
		$data['content_id'] 		= $seg4; //$this->uri->segment(4);
