...
echo form_open('ehr/ehr/index/ehr_pharmacy/ehr_pharmacy_supplier/phar_edit_drug_product/'.$form_purpose.'/drug/'.$supplier_id.'/'.$product_id, $attributes);
...
echo "\n<td>".anchor('ehr/ehr/index/ehr_pharmacy/ehr_pharmacy_supplier/phar_edit_drugsupplier_info/edit_supplier/drug/'.$supplier_id, "<strong>".$supplier_info[0]['supplier_name']."</strong>")."</td>";
echo "</td></tr>";
...
echo "\n<tr>";
    echo "\n<td width='25%'>Trade name (formulary) <font color='red'>*</font></td>";
...
echo $app_currency." <input type='text' name='wholesale_price' value='".set_value('wholesale_price',$init_wholesale_price)."' size='7' /> per pack";
if(!empty($init_packing)){
    echo " of ".$init_packing." ".$init_packing_form;
}
echo "</td></tr>";

echo "\n<tr><td>Standard Cost</td><td>";
echo form_error('ucost_std');
echo $app_currency." <input type='text' name='ucost_std' value='".set_value('ucost_std',$init_ucost_std)."' size='7' /> per pack";
if(!empty($init_packing)){
    echo " of ".$init_packing." ".$init_packing_form;
}
echo "</td></tr>";
...
echo $app_currency." <input type='text' name='retail_price' value='".set_value('retail_price',$init_retail_price)."' size='7' /> per pack";
if(!empty($init_packing)){
    echo " of ".$init_packing." ".$init_packing_form;
}
echo "</td></tr>";
...
echo $app_currency." <input type='text' name='retail_price_2' value='".set_value('retail_price_2',$init_retail_price_2)."' size='7' /> per pack</td></tr>";

echo "\n<tr><td>Retail Price 3</td><td>";
echo form_error('retail_price_3');
echo $app_currency." <input type='text' name='retail_price_3' value='".set_value('retail_price_3',$init_retail_price_3)."' size='7' /> per pack</td></tr>";

echo "\n<tr><td>Commonly Used</td><td>";
echo form_error('commonly_used');
echo "<input type='text' name='commonly_used' value='".set_value('commonly_used',$init_commonly_used)."' size='3' /> max. 32767</td></tr>";
...
if(isset($formulary_info[0])){
    ...
    echo "\n<tr>";
        ...
        echo "\n<td>".anchor('ehr/ehr/index/ehr_utilities/ehr_utilities/util_edit_drugcode/edit_formulary/'.$formulary_info[0]['drug_formulary_id'], $drugcode_info[0]['trade_name'])."</td>";
    echo "\n</tr>";
    echo "\n<tr>";
        echo "\n<td>Generic name</td>";
        //echo "<td>".$formulary_info[0]['generic_name']."</td>";
        echo "\n<td>".anchor('ehr/ehr/index/ehr_utilities/ehr_utilities/util_edit_drugformulary/edit_formulary/'.$formulary_info[0]['drug_formulary_id'], $formulary_info[0]['generic_name'])."</td>";
    echo "\n</tr>";
    echo "\n<tr>";
