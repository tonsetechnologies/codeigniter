...
if($debug_mode) {
    ...
    print "\n<br />init_date_delivery = " . $init_date_delivery;
    print "\n<br />timestamp lmp_edd = " . strtotime($antenatal_info[0]['lmp_edd']);
    echo "\n<pre>";
...
echo "\n\n<div id='page_title' align='center'><h1>".$this->lang->line('patients_edit_his_antenatal_delivery_html_title')."</h1></div>";
...
echo form_open('indv/indv/index/indv_antenatal/ehr_individual_antenatal/edit_history_antenatal_delivery/'.$form_purpose.'/'.$patient_id.'/'.$antenatal_id.'/'.$antenatal_delivery_id, $attributes);
...
echo form_error('delivery_type');
    ...
    echo "\n<option value='Not applicable' ".($init_delivery_type=='Not applicable'?' selected=\'selected\'':'').">Not applicable</option>";
    echo "\n</select>";
echo "</td></tr>\n";
...
echo "\n<tr>";
    ...
    if(isset($init_child_id)) {
        echo "\n&nbsp;&nbsp;";
        echo anchor('indv/indv/index/indv_overview/ehr_individual/individual_overview/'.$init_child_id, "<strong>View Child</strong>", 'target="_blank"');
    }
    echo "\n</td>";
echo "\n</tr>";
} else {
    echo form_hidden('child_id', '');
...
if($init_baby_condition<>'Dead'){
    echo "<p><span class='simple_button_yellow' ><strong>";
    echo anchor('indv/indv/index/indv_overview/ehr_individual/edit_patient/new_patient/new_patient/'.$init_date_delivery, 'Register Baby as New Patient', 'target="_blank"');
    echo "</strong></span></p>";
}
