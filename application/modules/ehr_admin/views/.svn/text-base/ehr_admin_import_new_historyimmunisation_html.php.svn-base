...
echo "\n\n<div id='page_title' align='center'><h1>".$this->lang->line('admin_import_new_historyimmunisation_html_title')."</h1></div>";
...
echo "\n<fieldset>";
echo "<legend>IMPORT IMMUNISATION HISTORIES</legend>";
echo "\nBased on the file named <a href='http://".$_SERVER["SERVER_ADDR"]."/".$app_folder."-uploads/imports_history/".$filename."' target='_blank'>$filename</a> the following patient records are available for importing:\n";
$attributes =   array('class' => 'select_form', 'id' => 'import_form');
echo form_open('ehr/ehr/index/ehr_admin/ehr_admin_edi_import/admin_import_new_historyimmunisationdone/import_immunisation/'.$filename, $attributes);
...
echo "\n<tr>";
    ...
    echo "\n<th width='50'>Vaccination date</th>";
...
if(isset($unsynched_list)){
    ...
    foreach ($unsynched_list as $unsynched_item){
        echo "\n<tr>";
        echo "\n<td><input type='checkbox' name='s$rownum' value='".$unsynched_item['patient_immunisation_id']."' checked='checked' /></td>";
        echo "\n<td>".$rownum.".</td>";
        ...
        echo "\n<td>".$unsynched_item['immunisation_date']."</td>";
        echo "\n<td>".$unsynched_item['vaccine_short']."</td>";
