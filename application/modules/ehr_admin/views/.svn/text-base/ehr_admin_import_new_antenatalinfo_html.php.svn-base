...
echo "\n\n<div id='page_title' align='center'><h1>".$this->lang->line('admin_import_new_antenatalinfo_html_title')."</h1></div>";
...
echo "\n<fieldset>";
echo "<legend>IMPORT ANTENATAL INFO</legend>";
...
echo form_open('ehr/ehr/index/ehr_admin/ehr_admin_edi_import/admin_import_new_antenatalinfodone/import_antenatal/'.$filename, $attributes);
...
if(isset($unsynched_list)){
    ...
    foreach ($unsynched_list as $unsynched_item){
        echo "\n<tr>";
        echo "\n<td><input type='checkbox' name='s$rownum' value='".$unsynched_item['event_id']."' checked='checked' /></td>";
        echo "\n<td>".$rownum.".</td>";
        ...
        echo "\n<td>".$unsynched_item['lmp']."</td>";
        echo "\n<td>(".$unsynched_item['lmp_edd'].")</td>";
