...
echo "\n\n<div id='page_title' align='center'><h1>".$this->lang->line('admin_export_historyimmunisation_html_title')."</h1></div>";


echo "\n<fieldset>";
echo "<legend>UNSYNCHED IMMUNISATION HISTORIES</legend>";
$attributes =   array('class' => 'select_form', 'id' => 'export_form');
echo form_open('ehr/ehr/index/ehr_admin/ehr_admin_edi_export/admin_export_new_historyimmunisation', $attributes);
...
if(isset($unsynched_list)){
    ...
    foreach ($unsynched_list as $unsynched_item){
        echo "\n<tr>";
        echo "\n<td><input type='checkbox' name='s$rownum' value='".$unsynched_item['patient_immunisation_id']."-.-".$unsynched_item['vaccine_short']."-.-".$unsynched_item['patient_id']."-.-".$unsynched_item['name']."-.-".$unsynched_item['name_first']."-.-".$unsynched_item['birth_date']."-.-".$unsynched_item['gender']."-.-".$unsynched_item['date']."-.-".trim($unsynched_item['immunisation_id'])."-.-".$unsynched_item['synch_out']."' checked='checked' /></td>";
        echo "\n<td>".$rownum.".</td>";
        ...
        echo "\n<td>";
     	echo anchor('indv/indv/index/indv_overview/ehr_individual/past_con_details/print_hardcopy/'.$unsynched_item['patient_id'].'/'.$unsynched_item['patient_immunisation_id'], $unsynched_item['date'], 'target="_blank"');
        echo "\n</td>";
...
if(isset($synched_list)){
    ...
    foreach ($synched_list as $synched_item){
        ...
        echo "\n<td>".$rownum.".</td>";
        echo "\n<td>".anchor('indv/indv/index/indv_overview/ehr_individual/individual_overview/overview/'.$synched_item['patient_id'], "<strong>".$synched_item['name']."</strong>, ".$synched_item['name_first'], 'target="_blank"')."</td>";
