...
echo "\n\n<div id='page_title' align='center'><h1>".$this->lang->line('admin_export_new_antenatalcheckup_html_title')."</h1></div>";


echo "\n<fieldset>";
echo "<legend>EXPORT EPISODES</legend>\n";
$attributes =   array('class' => 'select_form', 'id' => 'export_form');
echo form_open('ehr/ehr/index/ehr_admin/ehr_admin_edi_export/admin_export_new_antenatalcheckup_done', $attributes);
...
if(isset($unsynched_list)){
    ...
    foreach ($unsynched_list as $unsynched_item){
        echo "\n<tr>";
        echo "\n<td><input type='checkbox' name='s$rownum' value='".$unsynched_item['antenatal_followup_id']."-.-".$unsynched_item['antenatal_id']."-.-".$unsynched_item['patient_id']."' checked='checked' /></td>";
        echo "\n<td>".$rownum.".</td>";
        ...
        echo "\n</td>";
        echo "\n<td>".date("Y-m-d H:i",(int)$unsynched_item['synch_out'])."</td>";
        echo "\n</tr>";
        $rownum++;
    }//endforeach;
}
