...
echo "\n\n<div id='page_title' align='center'><h1>".$this->lang->line('admin_import_new_episodes_html_title')."</h1></div>";
...
echo "\nBased on the file named <a href='http://".$_SERVER["SERVER_ADDR"]."/".$app_folder."-uploads/imports_consult/".$filename."' target='_blank'>$filename</a> the following patient records are available for importing:\n";

$attributes =   array('class' => 'select_form', 'id' => 'import_form');
echo form_open('ehr/ehr/index/ehr_admin/ehr_admin_edi_import/admin_import_new_episodesdone/import_episodes/'.$filename, $attributes);
...
if(isset($unsynched_list)){
    ...
    foreach ($unsynched_list as $unsynched_item){
        ...
        echo "\n<td>".$rownum.".</td>";
        echo "\n<td>".anchor('ehr/ehr/index/ehr_admin/ehr_admin_edi/admin_export_details_patients/import_episode/'.$unsynched_item['patient_id'], "<strong>".$unsynched_item['patient_name']."</strong>, ".$unsynched_item['name_first'], 'target="_blank"')."</td>";
