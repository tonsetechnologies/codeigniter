...
echo "\n\n<div id='page_title' align='center'><h1>".$this->lang->line('admin_import_logins_html_title')."</h1></div>";
...
if(isset($fileList)){
    ...
    foreach ($fileList as $unsynched_item){
	    if (is_file("$directory/$unsynched_item") && $unsynched_item != '.' && $unsynched_item != '..' && (substr($unsynched_item,0,12)=="users_logins")) {
		    echo "\n<tr>";
            echo "\n<td>".$rownum.".</td>";
            //echo "\n<td><a href='/thirra-uploads/imports_consult/".$unsynched_item."' target='_blank'>".$unsynched_item."</td>";
            echo "\n<td><a href='http://".$_SERVER["SERVER_ADDR"]."/".$app_folder."-uploads/imports_system/".$unsynched_item."' target='_blank'>$unsynched_item</a></td>";
            echo "\n<td align='right'>".filesize("$directory/$unsynched_item")."</td>";
            echo "\n<td> &nbsp; ".anchor('ehr/ehr/index/ehr_admin/ehr_admin_edi_import/admin_import_new_logins/import_logs/'.$unsynched_item, 'Import')."</td>";
            echo "\n</tr>";
            $rownum++;
	    }
    }//endforeach;
