...
echo form_open('ehr/ehr/index/ehr_admin/ehr_admin/edit_systemuser/'.$form_purpose.'/'.$user_id);
...
echo "\n<input type='text' name='username' id='username' value='".set_value('username',$username)."' size='15' /></td></tr>";
...
if($user_id == ""){
    ...
    echo form_hidden('home_clinic', $home_clinic);
}

echo "\n<tr><td width='200'>Staff Name <font color='red'>*</font></td><td>";
echo form_error('staff_name');
echo "<input type='text' name='staff_name' value='".set_value('staff_name',$staff_name)."' size='30' /> (include Dr. XYZ for doctors)</td></tr>";
...
echo form_error('nationality');
    ...
    echo "\n<option value='Other' ".($nationality=='Other'?' selected=\'selected\'':'').">Other</option>";
    echo "\n</select>";
echo "</td></tr>\n";
...
/*
echo "\n<fieldset>";
echo "<legend>Authorised Clinics</legend>";
echo "\n<table>";
$row_num =   1;
echo "\n<tr>";
    echo "\n<th>No.</th>";
...
foreach($user_clinics as $clinic_item){
    ...
    echo "<td>".$row_num.". </td>";
    echo "\n<td>";
    echo "<input type='radio' name='home_clinic' value='".$clinic_item['clinic_info_id']."' ".($home_clinic==$clinic_item['clinic_info_id']?"checked='checked'":"")." >&nbsp;</input>";
    echo "</td>";
...
if($user_id <> ""){
    echo "\n<fieldset>";
    echo "<legend>Authorised Clinics (AJAX)</legend>";
    echo "\n<div id='ajax_change_home'>";
    echo "</div>"; //id='ajax_spouse_info'
    echo "\n<div id='ajax_add_clinic'>";
    echo "</div>"; //id='add_new_clinic'
    echo "\n<div id='submit_spouse_info'>";
    echo "<input type='submit' value='Add Clinic' />";
...
$siteurl_spouse_info    =   site_url()."/ehr_ajax/add_clinic_staff/";
?>
<script  type="text/javascript">
    ...
    function ajax_submit_spouse_info(ajax_outcome) {
        ...
        var patient_id = "<?php echo $home_clinic; ?>";
        var ajax_username = $('#username').val();
        var data = 'ajax_outcome='+ajax_outcome+'&patient_id='+patient_id+'&ajax_username='+ajax_username;
        //alert(siteurl+data);
        $.ajax({
          type:'POST',
          url:siteurl,
          ...
          success: function (result) {
            ...
            $('#ajax_change_home').html(multi_part[0]);
            $('#ajax_add_clinic').html(multi_part[1]);
          }
        }
    )}
