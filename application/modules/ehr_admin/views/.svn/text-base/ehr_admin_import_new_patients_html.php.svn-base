...
echo "\n\n<div id='page_title' align='center'><h1>".$this->lang->line('admin_import_new_patients_html_title')."</h1></div>";
...
if(count($imported_before) > 0){
    ...
    echo "\n<p class='section_body'>";
        ...
        foreach ($imported_before as $import_item){
            echo "<tr>";
            echo "<td>".anchor('ehr/ehr/index/ehr_admin/ehr_admin/edit_staff_category/edit_category/'.$import_item['data_synch_log_id'], date("Y-m-d H:i",$import_item['import_when']))."</td>";
...
echo "\n<fieldset>";
echo "<legend>IMPORT PATIENTS</legend>\n";
echo "\nBased on the file named <a href='http://".$_SERVER["SERVER_ADDR"]."/".$app_folder."-uploads/imports_patient/".$filename."' target='_blank'>$filename</a> the following patient records are available for importing:\n";
$attributes =   array('class' => 'select_form', 'id' => 'export_form');
echo form_open('ehr/ehr/index/ehr_admin/ehr_admin_edi_import/admin_import_new_patientsdone/import_patients/'.$filename, $attributes);
...
if(isset($unsynched_list)){
    foreach ($unsynched_list as $unsynched_item){
        echo "\n<tr>";
        echo "\n<td><input type='checkbox' name='s$rownum' value='".$unsynched_item['patient_id']."' checked='checked' /></td>";
        echo "\n<td>".$rownum.".</td>";
        echo "\n<td>".anchor('indv/indv/index/indv_overview/ehr_individual/individual_overview/overview/'.$unsynched_item['patient_id'], "<strong>".$unsynched_item['patient_name']."</strong>, ".$unsynched_item['name_first'], 'target="_blank"')."</td>";
        echo "\n<td>".$unsynched_item['birth_date']."</td>";
        echo "\n<td>".$unsynched_item['gender']."</td>";
        echo "\n<td>(".$unsynched_item['patient_id'].")</td>";
