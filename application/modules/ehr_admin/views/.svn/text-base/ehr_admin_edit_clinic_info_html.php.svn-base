...
echo form_open('ehr/ehr/index/ehr_admin/ehr_admin/admin_edit_clinic_info/'.$form_purpose.'/'.$clinic_info_id, $attributes);
...
echo "<input type='text' name='established' value='".set_value('established',$init_established)."' size='10' /> YYYY-MM-DD</td></tr>";
...
echo "\n<tr><td>Health Department</td><td>";
echo form_error('health_department_id');
//echo "<input type='text' name='health_department_id' value='".set_value('health_department_id',$init_health_department_id)."' size='10' />";
        echo "\n<select name='health_department_id' class='normal' id='health_department_id'>";
            echo "\n<option value='' >Please select one</option>";
            foreach($healthdept_list as $healthdept)
            {
	            echo "\n<option value='".$healthdept['health_department_id']."' ";
                if(isset($init_health_department_id)) {
                    echo ($init_health_department_id==$healthdept['health_department_id'] ? " selected='selected'" : "");
                }
                echo ">".$healthdept['office_name']." ".$healthdept['district']."</option>";
            } //foreach
        echo "</select>";
echo "</td></tr>";
...
//echo "<input type='text' name='remarks' value='".set_value('remarks',$init_remarks)."' size='100' />";
echo "\n<textarea class='normal' name='remarks' id='remarks' cols='40' rows='4'>".$init_remarks."</textarea>";
echo "</td></tr>";
...
echo "\n<fieldset>";
echo "<legend>CLINIC STAFF</legend>";
    echo "\n<p class='section_body'>";
        echo "<p>";
        echo anchor('ehr/ehr/index/ehr_admin/ehr_admin/edit_systemuser/new_systemuser', 'Add New User to Database');
        echo "</p>";
        ...
        echo "</tr>";
        $row_num    =   1;
        foreach ($staff_list as $user_item){
            echo "<tr>";
            echo "<td align='right'>".$row_num.". </td>";
