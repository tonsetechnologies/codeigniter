...
echo "\n\n<div id='page_title' align='center'><h1>".$this->lang->line('admin_export_new_antenatalinfo_html_title')."</h1></div>";


echo "\n<fieldset>";
echo "<legend>EXPORT EPISODES</legend>\n";
$attributes =   array('class' => 'select_form', 'id' => 'export_form');
echo form_open('ehr/ehr/index/ehr_admin/ehr_admin_edi_export/admin_export_new_antenatalinfo_done', $attributes);
...
if(isset($unsynched_list)){
    ...
    foreach ($unsynched_list as $unsynched_item){
        echo "\n<tr>";
        echo "\n<td><input type='checkbox' name='s$rownum' value='".$unsynched_item['antenatal_id']."-.-".$unsynched_item['patient_id']."' checked='checked' /></td>";
        echo "\n<td>".$rownum.".</td>";
        ...
        echo "\n<td>";
     	echo anchor('indv/indv/index/indv_overview/ehr_individual/past_con_details/print_hardcopy/'.$unsynched_item['patient_id'].'/'.$unsynched_item['antenatal_id'], $unsynched_item['date'], 'target="_blank"');
        echo "\n</td>";
