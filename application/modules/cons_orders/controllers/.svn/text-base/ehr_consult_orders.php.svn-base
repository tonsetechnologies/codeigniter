 ...
 */
class Ehr_consult_orders extends MX_Controller 
{
    ...
    function edit_lab($seg3,$seg4,$seg5,$seg6)
    {
        ...
        if(count($_POST)) {
            ...
            if(isset($_POST['enter_tests'])) { 
				...
				if($data['enter_tests'] == 'TRUE'){
					...
					$data['results_info']  = $this->morders_lab_rdb->get_one_lab_orderresult($data['lab_package_id']);
            ...
            if ($data['form_purpose'] == "new_lab") {
				...
				$data['results_info']  = $this->morders_lab_rdb->get_one_lab_orderresult($data['lab_order_id']);
        ...
        foreach($data['lab_list'] as $ordered){
            $has_details  = $this->morders_lab_rdb->get_one_lab_orderresult($ordered['lab_order_id']);
        ...
        $data['packages_list']= $this->morders_lab_rdb->get_lab_packages_list($data['init_location_id']);
		$data['supplier_list']  = $this->morders_lab_rdb->get_supplier_by_labpackage($data['lab_package_id']);
		...
		if(($data['enter_tests'] == TRUE) && ($data['lab_package_id'] <> "none")){
			...
			$data['packages_list']= $this->morders_lab_rdb->get_one_lab_package($data['lab_package_id']);
		...
		*/
		...
		if ($this->form_validation->run('edit_lab_order') == FALSE){
            ...
            if($data['form_purpose'] == "new_lab") {
                ...
                $upd_lab_array['staff_id']        = $this->session->userdata('staff_id');
    ...
    // ------------------------------------------------------------------------
    function consult_delete_lab($seg3,$seg4,$seg5,$seg6) 
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        ...
        $data['ordering_status']    =   $seg6; //$this->uri->segment(6);
    ...
    function edit_imaging($seg3,$seg4,$seg5,$seg6)
    {
		$this->load->model('morders_wdb');
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        ...
        if(count($_POST)) {
            ...
            if ($data['form_purpose'] == "new_imaging") {
                ...
                $data['order_info'] = $this->morders_imaging_rdb->get_one_imaging_product($data['order_id']); //
        ...
        $data['now_date']           =   date("Y-m-d",$data['now_id']);
        //$data['init_patient_id']    =   $patient_id;
		//$data['imaging_info']  = $this->morders_imaging_rdb->get_one_imaging_product($data['order_id']);

        $data['imaging_categories'] = $this->morders_imaging_rdb->get_imaging_categories();
		$data['product_list'] = $this->morders_imaging_rdb->get_imaging_product_by_category($data['imaging_category'],$data['init_location_id']);
        if(isset($data['product_id'])){
		    $data['supplier_list'] = $this->morders_imaging_rdb->get_supplier_by_product($data['product_id']);
        } else {
        ...
        */
		...
		if ($this->form_validation->run('edit_imaging_order') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_body   =   "ehr_edit_imaging_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
            ...
            if($data['form_purpose'] == "new_imaging") {
                ...
                $ins_imaging_array['staff_id']           = $this->session->userdata('staff_id');
                ...
                $upd_imaging_array['staff_id']           = $this->session->userdata('staff_id');
