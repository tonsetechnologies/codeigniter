 ...
 */
class Ehr_consult extends MX_Controller 
{
    ...
    // ------------------------------------------------------------------------
    function consult_new($seg3,$seg4,$seg5,$seg6)
    {
		...
		$data['summary_ref']        =   $seg6; //$this->uri->segment(4);
		$data['patient_info']       =   $this->memr_rdb->get_patient_demo($data['patient_id']);
        $data['patcon_info']        =   $this->memr_rdb->get_patcon_details($data['patient_id'],NULL,$this->session->userdata('location_id'));
		$data['patient_past_con'] = $this->mindiv_rdb->get_pastcons_list($data['patient_id']);
		$this->load->vars($data);
        ...
        if($data['patcon_info']['summary_id']=="new_summary"){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_body   =   "ehr_indv_startconsult_html";
            ...
            $this->session->set_flashdata('data_activity', 'Continuing an already open session.');
            $new_page = base_url()."index.php/cons/cons/index/cons_progress/ehr_consult/consult_episode/edit_episode/".$data['patient_id']."/".$data['patcon_info']['summary_id'];
            header("Status: 200");
            header("Location: ".$new_page);
    ...
    // ------------------------------------------------------------------------
    function consult_episode($seg3,$seg4,$seg5,$seg6)
    {
        ...
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        $data['mod_billing']		=	$this->config->item('mod_billing');
		$data['summary_ref']        =   $seg6; //$this->uri->segment(4);
        ...
        if(count($_POST) && $data['summary_ref']=="new_summary"){ 
            // User pressed Button to Create New Episode 
            $data['came_from']  = "New button";
            $data['patient_id'] = $this->input->post('patient_id');
            $data['summary_id'] = $this->input->post('summary_id');
            $data['init_location_id']   =   $this->session->userdata('location_id'); 
            $data['patcon_info']    = $this->memr_rdb->get_patcon_details($data['patient_id'],NULL,$data['init_location_id']);
            if($data['patcon_info']['summary_id'] <> "new_summary"){
                // Possibly user clicked Button twice
                $data['came_from']  = "New button twice";
                $data['patient_id']     =   $seg4; //$this->uri->segment(3);
                $data['summary_id']     =   $data['patcon_info']['summary_id'];
                $data['date_ended']     =   $data['now_date'];
                $data['time_ended']     =   $data['now_time'];
                $data['consult_notes']  =   $data['patcon_info']['consult_notes'];
                $data['external_ref']   =   $data['patcon_info']['external_ref'];
                $data['note_subjective']=   $data['patcon_info']['note_subjective'];
                $data['note_objective'] =   $data['patcon_info']['note_objective'];
                $data['note_assessment']=   $data['patcon_info']['note_assessment'];
                $data['note_plan']     =   $data['patcon_info']['note_plan'];
                $this->session->set_flashdata('data_activity', 'Continuing an already opened session.');
                
            } else {
                // Create new episode - First Press
                $this->load->model('mconsult_wdb');
                $ins_episode_array   =   array();
                $ins_episode_array['staff_id']              =   $this->session->userdata('staff_id'); 
                $ins_episode_array['adt_id']                =   $data['summary_id']; //$data['now_id'];
                $ins_episode_array['summary_id']            =   $data['summary_id']; //$data['now_id'];
                ...
                $data['consult_notes']  =   "";
                $data['external_ref']   =   "";
                $data['note_subjective']=   "";
                $data['note_objective'] =   "";
                $data['note_assessment']=   "";
                $data['note_plan']      =   "";
            }
        } elseif(count($_POST) && $data['form_purpose']=="edit_episode"){
            $data['came_from']  = "Close episode";
            ...
            $data['note_subjective']=   $this->input->post('note_subjective');
            $data['note_objective'] =   $this->input->post('note_objective');
            $data['note_assessment']=   $this->input->post('note_assessment');
            $data['note_plan']      =   $this->input->post('note_plan');
            $data['dry_run']        =   $_POST['dry_run']; // dry_run == TRUE will not make any new insertion or update to DB
        } else { // User arrived from a link, e.g. sidebar
			// User resumed episode since no POSTing
            $data['came_from']      =   "A link";
            $data['patient_id']     =   $seg4; //$this->uri->segment(3);
            $data['summary_id']     =   $seg5; //$this->uri->segment(4);
                $data['patcon_info']    = $this->memr_rdb->get_patcon_details($data['patient_id'],NULL,$this->session->userdata('location_id'));
                $data['summary_id']     =   $data['patcon_info']['summary_id'];
        ...
        $data['physical_info']  = $this->mcons_rdb->get_patcon_physical($data['summary_id']);
        ...
        $data['procedure_list'] = $this->morders_procedure_rdb->get_patcon_procedures($data['summary_id']);
        $data['prescribe_list'] = $this->memr_rdb->get_patcon_prescribe($data['summary_id']);
        $data['dispense_list']  = $this->memr_rdb->get_patcon_dispense($data['summary_id']);
        ...
        */
		...
		if ($this->form_validation->run('edit_episode') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_banner =   "ehr/banner_ehr_conslt_html";
                $new_sidebar=   "ehr/sidebar_ehr_patients_conslt_html";
                $new_body   =   "ehr_indv_consult_html";
                $new_footer =   "ehr/footer_emr_html";
            }
            //$this->load->view($new_header);			
            //$this->load->view($new_banner);			
            ...
            $this->load->model('morders_procedure_wdb');
                ...
                $end_episode_array['signed_by']          = $this->session->userdata('staff_id');
                $end_episode_array['consult_summary']    = $data['consult_notes'];
                $end_episode_array['note_subjective']    = $data['note_subjective'];
                $end_episode_array['note_objective']     = $data['note_objective'];
                $end_episode_array['note_assessment']    = $data['note_assessment'];
                $end_episode_array['note_plan']          = $data['note_plan'];
                $end_episode_array['location_end']       = $this->session->userdata('location_id');
				...
				if(count($data['lab_list']) > 0 ){
					...
					while($k < count($data['lab_list'])){
						...
						$data['results_info']  = $this->morders_lab_rdb->getsimple_one_lab_results($data['lab_list'][$k]['lab_order_id']);
						...
						if(count($data['results_info']) > 0){
							...
							for($j=1; $j <= $data['num_of_tests']; $j++){
								...
								$ins_test_array['staff_id']	=	$this->session->userdata('staff_id');
				...
				if(count($data['imaging_list']) > 0 ){
					...
					while($l < count($data['imaging_list'])){
						...
						//echo "l=".$l;
						$data['results_info'] =	array();
						$data['results_info']  = $this->morders_imaging_rdb->getsimple_one_imaging_result($data['imaging_list'][$l+1]['order_id']);
						//echo "<br />Results_count=". count($data['results_info']);
						//echo "<pre>";
						//print_r($data['results_info']);
						//echo "</pre>";
               ...
               // Change status of prescription records
				if(count($data['prescribe_list']) > 0 ){
					...
					while($m < count($data['prescribe_list'])){
                        ...
                        $ins_prescribe_array['staff_id']         = $this->session->userdata('staff_id');
               ...
               // Change status of dispense records
				if(count($data['dispense_list']) > 0 ){
					//echo "Has a dispense.";
					// Was there a result?
					$m=0; 
					while($m < count($data['dispense_list'])){
						$upd_prescribe_array   =   array();
						$upd_prescribe_array['queue_id']     = $data['dispense_list'][$m+1]['queue_id'];
                        $data['queue_id']       =   $upd_prescribe_array['queue_id'];
						$upd_prescribe_array['status']       = "Pending";
						$upd_prescribe_array['prescribe_method']= "dispense";
                        if($data['dry_run'] == "FALSE"){
                            $upd_prescribe_data =   $this->mconsult_wdb->update_prescription($upd_prescribe_array);
                        }
                        ...
                        // Insert patient_medication
                        //echo "Retrieve prescription";
                        $data['prescribe_info'] = $this->memr_rdb->get_patcon_dispense($data['summary_id'],$data['queue_id']);
                        $data['drug_system']        =   $data['prescribe_info'][1]['formulary_system'];
                        $data['drug_formulary_id']  =   $data['prescribe_info'][1]['drug_formulary_id'];
                        $data['generic_name']       =   $data['prescribe_info'][1]['generic_name'];
                        $data['drug_code_id']       =   $data['prescribe_info'][1]['drug_code_id'];
                        ...
                        $ins_prescribe_array['staff_id']         = $this->session->userdata('staff_id');
                        //$ins_prescribe_array['prescript_queue_id']= $data['queue_id'];                        
                        $ins_prescribe_array['dispense_queue_id']= $data['queue_id'];                        
                        $ins_prescribe_array['drug_formulary_id']= $data['drug_formulary_id'];                        
                        $ins_prescribe_array['dose']             = $data['dose'];
                        $ins_prescribe_array['dose_form']        = $data['dose_form'];
                        $ins_prescribe_array['frequency']        = $data['frequency'];
						...
						$m++;
					}//while($l < count($data['dispense_list']))					
				} //endif(count($data['dispense_list']) > 0 )
				
               // Change status of procedure order records
				if(count($data['procedure_list']) > 0 ){
					//echo "Has a procedure order.";
					// Was there a result?
                    if($data['debug_mode']){
                        print_r($data['procedure_list']);  
                    }
					$m=0; 
					while($m < count($data['procedure_list'])){
                        // Insert patient_past_procedure
                        //echo "Retrieve procedure order";
                        $data['procedure_info'] = $this->morders_procedure_rdb->get_procorder_details($data['procedure_list'][$m]['order_id']);                        
                        $ins_procedure_array   =   array();
                        $ins_procedure_array['past_procedure_id']     = $data['procedure_info']['order_id'];
                        $ins_procedure_array['patient_id']       = $data['patient_id'];
                        $ins_procedure_array['staff_id']         = $this->session->userdata('staff_id');
                        $ins_procedure_array['procedure_date']     = $data['date_ended'];
                        $ins_procedure_array['date_precision']     = "F";
                        $ins_procedure_array['procedure_notes']     = $data['procedure_info']['notes'];
                        $ins_procedure_array['pcode1ext_id']     = $data['procedure_info']['pcode1ext_id'];
                        $ins_procedure_array['pcode1ext_code']     = $data['procedure_info']['pcode1ext_code'];
                        $ins_procedure_array['order_id']     = $data['procedure_info']['order_id'];
                        $ins_procedure_array['product_id']     = $data['procedure_info']['product_id'];
                        $ins_procedure_array['procedure_remarks']     = NULL;
                        if($ins_procedure_array['product_id'] <> NULL){
                            $ins_procedure_array['procedure_place']     = $this->session->userdata('clinic_shortname');
                        } else {
                            $ins_procedure_array['procedure_place']     = NULL;
                        }
                        if(empty($data['procedure_info']['procedure_outcome'])){
                            $ins_procedure_array['procedure_outcome']     = "Pending";
                        } else {
                            $ins_procedure_array['procedure_outcome']     = $data['procedure_info']['procedure_outcome'];
                        }
                        $ins_procedure_array['outcome_remarks']     = $data['procedure_info']['outcome_remarks'];
                        $ins_procedure_array['outcome_staff']     = $data['procedure_info']['outcome_staff'];
                        $ins_procedure_array['outcome_date']     = $data['procedure_info']['outcome_date'];
                        $ins_procedure_array['added_remarks']     = NULL;
						...
						$m++;
					}//while($l < count($data['procedure_list']))					
				} //endif(count($data['procedure_list']) > 0 )
				
                // Pop off from queue
                $data['queue_info'] = $this->mqueue_rdb->get_patients_queue($this->session->userdata('location_id'),$data['date_ended'],NULL,$data['patient_id']);
                ...
                // Billing
                // Set new appointment
            if($data['mod_billing'] === TRUE){
                $new_page = base_url()."index.php/cons/cons/index/cons_modulea/ehr_consult_modulea/edit_lab/new_lab/".$data['patient_id']."/".$data['summary_id'];
            } else {
                $new_page = base_url()."index.php/cons/cons/index/cons_progress/ehr_consult/close_episode/new_episode/".$data['patient_id']."/".$data['summary_id'];
            }
            header("Status: 200");
            if($data['dry_run'] == "FALSE"){
                header("Location: ".$new_page);
    ...
    function edit_reason_encounter($seg3,$seg4,$seg5,$seg6)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        ...
        if(count($_POST)) {
            ...
            $data['complaint_id']   = $seg6; //$this->uri->segment(6);
        ...
        $data['ccode1_chapters'] = $this->mcons_rdb->get_ccode_chapters();
		$data['ccode1_list'] = $this->mcons_rdb->get_ccode1_by_chapter($data['complaintChapter']);
		...
		if ($this->form_validation->run('edit_complaint') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_body   =   "ehr_edit_reason_encounter_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
            ...
            if($data['form_purpose'] == "new_complaints") {
                ...
                $ins_complaint_array['staff_id']           = $this->session->userdata('staff_id');
                ...
                $ins_complaint_array['remarks']            = "";//$data['remarks'];
                if($data['offline_mode']){
    ...
    // ------------------------------------------------------------------------
    function consult_delete_complaint($seg3,$seg4,$seg5,$seg6) 
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        $this->load->model('mconsult_wdb');
        $data['patient_id']         =   $seg4; //$this->uri->segment(3);
        $data['summary_id']         =   $seg5; //$this->uri->segment(4);
        $data['complaint_id']       =   $seg6; //$this->uri->segment(5);
    ...
    // ------------------------------------------------------------------------
    function edit_vitals($seg3,$seg4,$seg5,$seg6)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$this->load->model('mindiv_rdb');
    ...
    // ------------------------------------------------------------------------
    function edit_physical_exam($seg3,$seg4,$seg5,$seg6)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		//$this->load->library('form_validation');
		...
		if ($this->form_validation->run('edit_physical_exam') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_sidebar=   "ehr/sidebar_ehr_patients_conslt_wap";
                //$new_body   =   "ehr/emr_edit_vitals_wap";
                $new_body   =   "ehr_edit_physical_exam_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
            ...
            if($data['physical_exam_id'] == "new_physical") {
                ...
                $ins_physical_array['staff_id']             = $this->session->userdata('staff_id');
                ...
                $upd_physical_array['staff_id']             = $this->session->userdata('staff_id');
    ...
    function edit_referral($seg3,$seg4,$seg5,$seg6)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        ...
        if(count($_POST)) {
            ...
            $data['patient_id']         =   $seg4; //$this->uri->segment(4);
            $data['summary_id']         =   $seg5; //$this->uri->segment(5);
            $data['referral_id']        =   $seg6; //$this->uri->segment(6);
            $patient_id                 =   $seg4; //$this->uri->segment(4);
            //$data['patient_id']     =   $patient_id;
		...
		if ($this->form_validation->run('edit_referral') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_body   =   "ehr_edit_referral_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
            ...
            if($data['form_purpose'] == "new_referral") {
                ...
                $ins_referral_array['staff_id']           = $this->session->userdata('staff_id');
                ...
                $upd_referral_array['staff_id']           = $this->session->userdata('staff_id');
    ...
    // ------------------------------------------------------------------------
    function upload_pics_con($seg3,$seg4,$seg5)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        ...
        */
	    ...
	    if ( ! $this->upload->do_upload())
	    {
            ...
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_banner =   "ehr/banner_ehr_conslt_wap";
                $new_sidebar=   "ehr/sidebar_ehr_patients_conslt_wap";
                $new_body   =   "ehr_edit_upload_con_html";
                $new_footer =   "ehr/footer_emr_wap";
		    } else {
                $new_header =   "ehr/header_xhtml1-transitional";
                $new_banner =   "ehr/banner_ehr_conslt_html";
            ...
            */
            ...
            //}
            $ins_pics_array['staff_id']         =   $this->session->userdata('staff_id');
            ...
            $ins_pics_array['location_id']      =   $this->session->userdata('location_id');
            ...
            header("Status: 200");
            redirect('cons/cons/index/cons_progress/ehr_consult/upload_pics_con/new_file/'.$data['patient_id'].'/'.$data['summary_id'].'/new_referral','refresh');
            /*
                echo "<pre>";
                print_r($upload_data);
                $this->load->view('test_upload', $error);
    ...
    // ------------------------------------------------------------------------
    function con_externalmod($seg3,$seg4,$seg5)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		//$this->load->library('form_validation');
		...
		if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
			...
			$new_banner =   "ehr/banner_ehr_conslt_wap";
			$new_sidebar=   "ehr/sidebar_ehr_patients_conslt_wap";
			$new_body   =   "ehr_indv_con_externalmod_html";
			$new_footer =   "ehr/footer_emr_wap";
		} else {
			//$new_header =   "ehr/header_xhtml1-strict";
			$new_header =   "ehr/header_xhtml1-transitional";
	...
	// ------------------------------------------------------------------------
    function close_episode($seg3,$seg4)  // last page of consultation
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		...
		if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_banner =   "ehr/banner_ehr_conslt_wap";
            $new_sidebar=   "ehr/sidebar_ehr_patients_conslt_wap";
            $new_body   =   "ehr_close_episode_html";
            //$new_body   =   "emr_close_episode_wap";
            $new_footer =   "ehr/footer_emr_wap";
		} else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
    ...
    // ------------------------------------------------------------------------
    function consult_delete_session($seg3,$seg4,$seg5) 
    {
        $data['offline_mode']	=	$this->config->item('offline_mode');
        $data['debug_mode']		=	$this->config->item('debug_mode');
        $this->load->model('mconsult_wdb');
        ...
        $this->session->set_flashdata('data_activity', 'Session deleted.');
