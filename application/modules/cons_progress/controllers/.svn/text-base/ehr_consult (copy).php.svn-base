...
class Ehr_consult extends MX_Controller 
{
    ...
    // ------------------------------------------------------------------------
    function consult_new($seg3,$seg4,$seg5)
    {
    ...
    // ------------------------------------------------------------------------
    function consult_episode($seg3,$seg4,$seg5)
    {
		$this->load->model('mehr_wdb');
        ...
        if(count($_POST) && $data['form_purpose']=="new_episode"){ 
		...
		if ($this->form_validation->run('edit_episode') == FALSE){
            ...
            //echo "</pre>";
               ...
               // Change status of prescription records
                ...
                // Billing
                // Set new appointment
            $new_page = base_url()."index.php/cons/cons/index/cons_progress/ehr_consult/close_episode/".$data['patient_id']."/".$data['summary_id'];
            header("Status: 200");
            header("Location: ".$new_page);
