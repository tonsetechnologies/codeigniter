 ...
 */
class Ehr_consult_gem extends MX_Controller 
{
    ...
    // ------------------------------------------------------------------------
    function list_gem_submodules($seg3,$seg4,$seg5,$seg6)  // List drug packages
    {
        ...
        $data['gem_module_id']      =   $seg6; //$this->uri->segment(6);
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_body   =   "ehr_gem_list_submodules_html";
    ...
    // ------------------------------------------------------------------------
    function gem_select_agegroup($seg3,$seg4,$seg5,$seg6,$seg7)  // List age groups for selection
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
        ...
        $data['summary_id']         =   $seg5; //$this->uri->segment(5);
        $data['gem_module_id']      =   $seg6; //$this->uri->segment(6);
        $data['gem_submod_id']      =   $seg7; //$this->uri->segment(7);
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_sidebar=   "ehr/sidebar_ehr_patients_conslt_wap";
            //$new_body   =   "ehr/ehr_edit_immune_select_wap";
            $new_body   =   "ehr_gem_select_age_group_html";
            $new_footer =   "ehr/footer_emr_wap";
		} else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
    ...
    // ------------------------------------------------------------------------
    // GEM submodule form
    function gem_edit_consult($seg3,$seg4,$seg5,$seg6,$seg7)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        $data['form_purpose']       =   $seg3; //$this->uri->segment(3);
        $data['patient_id']         =   $seg4; //$this->uri->segment(4);
        $data['summary_id']         =   $seg5; //$this->uri->segment(5);
        $data['gem_submod_id']      =   $seg6; //$this->uri->segment(6);
        $data['gem_agegroup_id']    =   $seg7; //$this->uri->segment(7);
		$data['title'] = "GEM";
		$data['patient_info']       = $this->memr_rdb->get_patient_demo($data['patient_id']);
		...
		if ($this->form_validation->run('edit_consult_gem') == FALSE){
            ...
            if($data['form_purpose'] == "new_consult") {
                ...
                $ins_gem_array   =   array();
                $ins_gem_array['staff_id']           = $this->session->userdata('staff_id');
            ...
            //} elseif($data['form_purpose'] == "edit_consult") {
                ...
                $upd_gem_array   =   array();
                $upd_gem_array['staff_id']           = $this->session->userdata('staff_id');
            ...
            $new_page = base_url()."index.php/cons/cons/index/cons_gem/ehr_consult_gem/list_gem_submodules/list_submodules/".$data['patient_id']."/".$data['summary_id'].'/'.$data['submodule_info'][0]['gem_module_id'];
            header("Status: 200");
            header("Location: ".$new_page);
