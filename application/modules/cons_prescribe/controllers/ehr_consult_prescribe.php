 ...
 */
class Ehr_consult_prescribe extends MX_Controller 
{
    ...
    function edit_prescription($seg3,$seg4,$seg5,$seg6)
    {
        ...
        $data['mod_inventctrl']		=	$this->config->item('mod_inventctrl');
        ...
        if(count($_POST)) {
            ...
            */
            $data['prescribe_method']   =   "prescribe"; // Default value
            if(isset($_POST['mdrug_code_id'])) { 
                $data['mdrug_code_id']   =   $_POST['mdrug_code_id'];
                if(substr($data['mdrug_code_id'],0,2) == "d-"){
                    $data['prescribe_method']   =   "dispense";
                }
                $data['drug_code_id']   =   substr($data['mdrug_code_id'],2,10);
                $data['product_id']     =   substr($data['mdrug_code_id'],13,10);
            } else {
                $data['drug_code_id']   =   "none";
                $data['mdrug_code_id']  =   "";
            } //endif(isset($_POST['mdrug_code_id']))
            ...
            //$data['prescribe_method']= $_POST['prescribe_method'];
        } else {
            ...
            $data['prescribe_id']   =   $seg6; //$this->uri->segment(6);
            ...
            if ($data['form_purpose'] == "new_prescribe") {
                ...
                $data['indication']         =   "";
                $data['caution']            =   "";
                $data['prescribe_method']   =   "prescribe";
                $data['mdrug_code_id']      =   "";
            } else {
                if ($data['form_purpose'] == "edit_dispense") {
                    //echo "Edit dispense";
                    $data['prescribe_method']   =   "dispense";
                    $data['prescribe_info'] = $this->memr_rdb->get_patcon_dispense($data['summary_id'],$data['prescribe_id']);
                    $data['product_id']     =   $data['prescribe_info'][1]['product_id'];
                } else {
                ...
                $data['mdrug_code_id']      =   substr($data['prescribe_method'],0,1)."-".$data['drug_code_id']."-".$data['product_id'];
        ...
        if(isset($data['drug_formulary_id'])){
		    ...
		    $data['drugstock_list'] = $this->mpharma_rdb->get_drugstock_by_formulary($data['drug_formulary_id'],$data['location_id']);
            ...
            if(count($data['formulary_chosen'])>0){
                ...
                if(isset($data['drug_code_id'])){
                    $data['tradename_chosen'] = $this->mprescribe_rdb->get_one_drug_tradename($data['drug_code_id']);
        ...
        */
		...
		if ($this->form_validation->run('edit_prescribe') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_body   =   "ehr_edit_prescription_html";
            ...
            if($data['form_purpose'] == "new_prescribe") {
                ...
                $ins_prescribe_array['product_id']       = $data['product_id'];
                $ins_prescribe_array['prescribe_method'] = $data['prescribe_method'];
                /*
                echo "<pre>";
                print_r($ins_prescribe_array);
                echo "mdrug_code_id=".$data['mdrug_code_id'];
                echo "</pre>";
                */
            ...
            } else { //if($data['form_purpose'] == "edit_prescribe") {
                // Existing prescription record
                $upd_prescribe_array   =   array();
                $upd_prescribe_array['staff_id']        = $this->session->userdata('staff_id');
                ...
                $upd_prescribe_array['generic_drugname'] = $data['formulary_chosen']['generic_name'];
                $upd_prescribe_array['drug_tradename']   = $data['tradename_chosen']['trade_name'];
                $upd_prescribe_array['drug_code_id']     = $data['drug_code_id']; 
                $upd_prescribe_array['product_id']       = $data['product_id'];
                $upd_prescribe_array['prescribe_method'] = $data['prescribe_method'];
            ...
            $new_page = base_url()."index.php/cons/cons/index/cons_prescribe/ehr_consult_prescribe/edit_prescription/new_prescribe/".$data['patient_id']."/".$data['summary_id'];
            header("Status: 200");
            header("Location: ".$new_page);
    ...
    function edit_prescribe($seg3,$seg4,$seg5,$seg6)
    {
		$this->load->model('mutil_rdb');
		$this->load->model('mpharma_rdb');
        $data['offline_mode']		=	$this->config->item('offline_mode');
        ...
        if(count($_POST)) {
            ...
            $data['drug_formulary_id']       = $_POST['drug_formulary_id'];
            if(($data['button'] == "Search") || ($data['button'] == "Search Generic Name")){
            ...
            $data['patient_id']     =   $seg4; //$this->uri->segment(4);
            $data['summary_id']     =   $seg5; //$this->uri->segment(5);
            $data['queue_id']       =   $seg6; //$this->uri->segment(6);
		...
		if ($this->form_validation->run('edit_prescribe') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_body   =   "ehr_edit_prescribe_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
            ...
            //echo "<br />Insert record";
	            ...
	            $ins_prescribe_data       =   $this->mconsult_wdb->insert_new_prescribe($ins_prescribe_array);
                $this->session->set_flashdata('data_activity', 'Prescription added.');
            $new_page = base_url()."index.php/cons/cons/index/cons_prescribe/ehr_consult_prescribe/edit_prescribe/new_prescribe/".$data['patient_id']."/".$data['summary_id'];
            header("Status: 200");
            header("Location: ".$new_page);
    ...
    // ------------------------------------------------------------------------
    function consult_delete_prescription($seg3,$seg4,$seg5,$seg6) 
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['form_purpose']       =   $seg3; //$this->uri->segment(3);
        ...
        $data['order_id']           =   $seg6; //$this->uri->segment(4);
        
        // Delete records
        $del_rec_array['form_purpose']  = $data['form_purpose'];
        $del_rec_array['queue_id']      = $data['order_id'];
    ...
    // ------------------------------------------------------------------------
    function list_drug_packages($seg3,$seg4,$seg5)  // List drug packages
    {
		$this->load->model('mpharma_rdb');
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		...
		$data['patient_info']   = $this->memr_rdb->get_patient_demo($data['patient_id']);
        $data['patcon_info']    = $this->memr_rdb->get_patcon_details($data['patient_id']);
        $data['packages_list']      = $this->mpharma_rdb->get_drug_package_list(NULL, $data['location_id']);
		$data['allergy_list']   = $this->memr_rdb->get_drug_allergies('List',$data['patient_id']);
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_body   =   "ehr_consult_list_drug_package_html";
            $new_footer =   "ehr/footer_emr_wap";
		} else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
    ...
    function edit_drug_package($seg3,$seg4,$seg5,$seg6)
    {
		$this->load->model('mpharma_rdb');
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        ...
        $data['patient_id']         =   $seg4; //$this->uri->segment(4);
        $data['summary_id']         =   $seg5; //$this->uri->segment(5);
        $data['drug_package_id']    =   $seg6; //$this->uri->segment(6);
		...
		if ($this->form_validation->run('edit_drug_package') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_sidebar=   "ehr/sidebar_ehr_patients_conslt_wap";
                //$new_body   =   "ehr/ehr_edit_immune_select_wap";
                $new_body   =   "ehr_edit_drug_package_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
            ...
            if($data['form_purpose'] == "new_package") {
                ...
                $ins_drug_package['staff_id']       = $this->session->userdata('staff_id');
                ...
                $upd_drug_package['staff_id']       = $this->session->userdata('staff_id');
    ...
    // ------------------------------------------------------------------------
    function edit_immune_select($seg3,$seg4,$seg5,$seg6)  // patient immunisation
	{
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$this->load->model('mpharma_rdb');
        ...
        $data['patient_immunisation'] 	= $this->mprescribe_rdb->get_patient_immunisation($data['patient_id'],999,0);
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_sidebar=   "ehr/sidebar_ehr_patients_conslt_wap";
            //$new_body   =   "ehr/ehr_edit_immune_select_wap";
            $new_body   =   "ehr_edit_immune_select_html";
            $new_footer =   "ehr/footer_emr_wap";
		} else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
		...
		$this->load->view($new_body);			
		//$this->load->view($new_footer);			
    } // end of function edit_immune_select($id)


    // ------------------------------------------------------------------------
    // Categorised prescription form
    function edit_immune_prescribe($seg3,$seg4,$seg5,$seg6)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$this->load->model('mpharma_rdb');
        ...
        if(count($_POST)) {
            ...
            $data['patient_id']     =   $seg4; //$this->uri->segment(4);
            $data['summary_id']     =   $seg5; //$this->uri->segment(5);
            $data['vaccine_id']     =   $seg6; //$this->uri->segment(6);
            $patient_id             =   $seg4; //$this->uri->segment(4);
            $data['patient_id']     =   $patient_id;
            if ($data['form_purpose'] == "new_immune") {
                //echo "New prescription";
        ...
        $data['init_clinic_name']   =   NULL;
        //$data['init_patient_id']    =   $patient_id;
        $data['this_vaccine'] 	= $this->mprescribe_rdb->get_vaccines_list($data['patient_id'],999,0,$data['vaccine_id']);

        $data['vaccines_list'] 	= $this->mprescribe_rdb->get_vaccines_list($data['patient_id']);
		// This array can be refactored to be automatic
		$data['vaccination_table']	= array(0,1,2,3,4,5,6,12,18,24);		
        //$data['drug_systems'] = $this->memr_rdb->get_drug_system();
		...
		if ($this->form_validation->run('edit_prescribe') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_body   =   "ehr_edit_immune_prescribe_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
            ...
            $new_page = base_url()."index.php/cons/cons/index/cons_prescribe/ehr_consult_prescribe/edit_immune_select/new_immune/".$data['patient_id']."/".$data['summary_id'];
            header("Status: 200");
            header("Location: ".$new_page);
