...
echo "\n\n<div id='page_title' align='center'><h1>".$this->lang->line('orders_edit_procsupplier_info_html_title')."</h1></div>";

echo "\n<div class='flashdata'>".$this->session->flashdata('data_activity')."</div>";

$attributes =   array('class' => 'select_form', 'id' => 'consultation_form');
echo form_open('ehr/ehr/index/ehr_orders/ehr_orders_procedure/orders_edit_procsupplier_info/'.$form_purpose.'/proc/'.$supplier_id, $attributes);
...
if($form_purpose == "edit_supplier"){
    echo anchor('ehr/ehr/index/ehr_orders/ehr_orders_procedure/orders_edit_proc_product/new_product/proc/'.$supplier_id.'/new_product', '<strong>Add Package</strong>');
}
...
foreach ($packages_list as $packages_item){
    echo "<tr>";
    echo "<td>".anchor('ehr/ehr/index/ehr_orders/ehr_orders_procedure/orders_edit_proc_product/edit_product/proc/'.$supplier_id.'/'.$packages_item['product_id'], $packages_item['product_code'])."</td>";
    echo "<td align='center'>".$packages_item['commonly_used']."</td>";
    echo "<td>".$packages_item['product_name']."</td>";
    echo "<td><strong>".$packages_item['clinic_shortname']."</strong></td>";
    echo "<td align='right'>".$packages_item['supplier_cost']."</td>";
    echo "<td align='right'>".$packages_item['retail_price_1']."</td>";
    echo "</tr>";
}//endforeach;
echo "</table>";
echo "\n</fieldset>";
