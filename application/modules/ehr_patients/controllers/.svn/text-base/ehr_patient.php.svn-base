 ...
 */
class Ehr_patient extends MX_Controller 
{
    ...
    function patients_mgt($id=NULL)  // template for new classes
    {
		...
		$data['pending_referrals'] = $this->mrefer_rdb->get_list_referrals('Consulted', $data['location_id']);
		$this->load->vars($data);
		if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_body   =   "ehr_patients_mgt_html";
    ...
    function search_patient()
    {
		...
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
        $data['breadcrumbs']        =   breadcrumbs('ehr_patient/patients_mgt','Patients');    
        $data['location_id']    	= 	$this->session->userdata('location_id');//$this->session->userdata('location_id');
		...
		if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_banner =   "ehr/banner_ehr_html";
            $new_sidebar=   "ehr/sidebar_emr_patients_html";
            $new_body   =   "ehr_patients_searched_html";
            $new_footer =   "ehr/footer_emr_html";
		}
        if($data['user_rights']['section_patients'] < 100){
            $new_body   =   "ehr/ehr_access_denied_html";
    ...
    // ------------------------------------------------------------------------
	function patients_list($seg3,$seg4,$seg5,$seg6) 
    {	
        $data['offline_mode']		= $this->config->item('offline_mode');
        $data['debug_mode']		    = $this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
        $data['breadcrumbs']        =   breadcrumbs('ehr_patient/patients_mgt','Patients');    
        $data['location_id']    	= $this->session->userdata('location_id'); //$this->session->userdata('location_id');
		$data['patient_scope'] 		= $seg3; //$this->uri->segment(3);
		$data['list_sort'] 			= $seg4; //$this->uri->segment(4);
		$data['alphabet'] 			= $seg5; //$this->uri->segment(5);
/*		
...
*/
		...
		if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_body   =   "ehr_patients_list_wap";
            $new_footer =   "ehr/footer_emr_wap";
		} else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
            $new_banner =   "ehr/banner_ehr_html";
            $new_sidebar=   "ehr/sidebar_emr_patients_html";
            $new_body   =   "ehr_patients_list_html";
            $new_footer =   "ehr/footer_emr_html";
		}
        if($data['user_rights']['section_patients'] < 100){
            $new_body   =   "ehr/ehr_access_denied_html";
