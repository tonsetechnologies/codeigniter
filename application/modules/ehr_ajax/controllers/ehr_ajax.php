 ...
 */
class Ehr_ajax extends MX_Controller 
{
    ...
    function show_medication()
    {
        ...
        $data['medication_list'] 	= $this->mindiv_rdb->get_recent_medication($data['patient_id'],999,5);
        //get the record from the database
		$this->load->vars($data);
        $this->load->view("ajax_get_indvoverview_medication");
    ...
    function show_immunisation()
    {
        ...
        //the procedure outlined in the CI documentation
        //$results = $this->records->get_record($record_id);
        $data['vaccines_list'] 	= $this->mprescribe_rdb->get_patient_immunisation($data['patient_id'],999,6);
        //get the record from the database
        //echo "Well done".$data['patient_id'] ;
		$this->load->vars($data);
        $this->load->view("ajax_get_indvoverview_immunisation");
    }
    ...
    function get_address_state_names()
    {
        ...
        //echo "Well done".$data['patient_id'] ;
		$this->load->vars($data);
        $this->load->view("ajax_get_address_statenames");
    ...
    function get_address_district_names()
    {
        ...
        //echo "Well done".$data['patient_id'] ;
		$this->load->vars($data);
        $this->load->view("ajax_get_address_districtnames");
    ...
    function get_address_area_names()
    {
        ...
        //echo "Well done".$data['patient_id'] ;
		$this->load->vars($data);
        $this->load->view("ajax_get_address_areanames");
    ...
    function get_address_town_names()
    {
       ...
       //get the record from the database
        //echo "Well done".$data['patient_id'] ;
		$this->load->vars($data);
        $this->load->view("ajax_get_address_townnames");
    ...
    function get_address_village_names()
    {
       ...
       //get the record from the database
        //echo "Well done".$data['patient_id'] ;
		$this->load->vars($data);
        $this->load->view("ajax_get_address_villagenames");
    } //endfunction get_address_town_names()


    // ------------------------------------------------------------------------
    // Save consultation_details
    function update_consultation_details()
    {
        //if(isset($_POST['summary_id']) && $_POST['summary_id']<>""){
            $this->load->model('mconsult_wdb');
            $data['summary_id']             =   $this->input->post('summary_id');
            $data['ajax_outcome']           =   $this->input->post('ajax_outcome');
            $data['date_started']           =   $this->input->post('ajax_date_started');
            $data['time_started']           =   $this->input->post('ajax_time_started');
            $data['external_ref']           =   $this->input->post('ajax_external_ref');
            $data['init_consult_summary']   =   $this->input->post('ajax_consult_notes');
            $data['init_note_subjective']   =   $this->input->post('ajax_note_subjective');
            $data['init_note_objective']    =   $this->input->post('ajax_note_objective');
            $data['init_note_assessment']   =   $this->input->post('ajax_note_assessment');
            $data['init_note_plan']         =   $this->input->post('ajax_note_plan');
                // Update database
                $upd_patient_array['summary_id']        = $data['summary_id'];
                $upd_patient_array['date_started']      = $data['date_started'];
                $upd_patient_array['time_started']      = $data['time_started'];
                $upd_patient_array['external_ref']      = $data['external_ref'];
                //$upd_patient_array['contact_id']        = $data['init_contact_info']['contact_id'];        
                $upd_patient_array['consult_summary']   = $data['init_consult_summary'];
                $upd_patient_array['note_subjective']   = $data['init_note_subjective'];
                $upd_patient_array['note_objective']    = $data['init_note_objective'];
                $upd_patient_array['note_assessment']   = $data['init_note_assessment'];
                $upd_patient_array['note_plan']         = $data['init_note_plan'];
                $upd_patient_data       =   $this->mconsult_wdb->update_episode($upd_patient_array);
            
            $this->load->vars($data);
            $this->load->view("ajax_update_consultation_summary");
        //} else {
        //}
    } //endfunction update_consultation_details()
    ...
    function update_kin_details()
    {
        if(isset($_POST['patient_id']) && $_POST['patient_id']<>""){
            ...
            //echo "Well done".$data['patient_id'] ;
            $this->load->vars($data);
            $this->load->view("ajax_update_kin_details");
        } else {
        }
    ...
    // ------------------------------------------------------------------------
    // Gives authorisation ta access a clinic. add_clinic_staff
    function add_clinic_staff()
    {
        $data['ajax_home_clinic']           =   $this->input->post('patient_id');
        $data['ajax_username']              =   $this->input->post('ajax_username');
        //echo "Well done".$data['ajax_username'] ;
        $this->load->model('madmin_rdb');
		$data['user_clinics'] = $this->madmin_rdb->get_user_clinics($data['ajax_username']);
		$data['clinics_list'] = $this->madmin_rdb->get_clinics_list('All');
        //if(isset($_POST['patient_id']) && $_POST['patient_id']<>""){
            $this->load->model('majax_wdb');
            $data['patient_id']             =   $this->input->post('patient_id');
            $data['ajax_outcome']           =   $this->input->post('ajax_outcome');
            $data['init_home_clinic']         =   $this->input->post('ajax_home_clinic');
            // Retrieve contact_id of kin
            $data['init_contact_info'] 	    = $this->majax_rdb->get_kin_contact_info($data['patient_id']);
            if($data['ajax_outcome'] == "Updated"){
                // Update database
                $ins_patient_array['patient_id']        = $data['patient_id'];
                $ins_patient_array['ic_no']             = $data['init_home_clinic'];
                $ins_patient_array['contact_id']        = $data['init_contact_info']['contact_id'];        
                $ins_patient_data       =   $this->majax_wdb->insert_kin_demog_info($ins_patient_array);
            }
            ...
            //echo "Well done".$data['patient_id'] ;
            $this->load->vars($data);
            $this->load->view("ajax_update_clinic_staff");
        //} else {
        //}
    } //endfunction add_clinic_staff()
    ...
    function get_diagnosis_diagnosiscategory()
    {
        ...
        //echo "Well done".$data['patient_id'] ;
		$this->load->vars($data);
        $this->load->view("ajax_get_diagnosis_diagnosiscategory");
    ...
    function get_diagnosis_diagnosisext()
    {
        ...
        //echo "Well done".$data['patient_id'] ;
		$this->load->vars($data);
        $this->load->view("ajax_get_diagnosis_diagnosiscode");
    ...
    function get_prescription_druggenericnames()
    {
        ...
        //echo "Well done".$data['patient_id'] ;
		$this->load->vars($data);
        $this->load->view("ajax_get_prescription_druggenericnames");
     ...
     function get_prescription_drugtradenames()
    {
        ...
        $data['drugstock_list'] = $this->mpharma_rdb->get_drugstock_by_formulary($data['ajax_drug_formulary_id'],$data['location_id']);
        ...
        //echo "Well done".$data['patient_id'] ;
		$this->load->vars($data);
        $this->load->view("ajax_get_prescription_drugtradenames");
    ...
    function get_atc_info()
    {
		...
		$this->load->vars($data);
        $this->load->view("ajax_get_atc_info");
    ...
    function get_formulary_info()
    {
        ...
        //echo "Well done".$data['atc_code'] ;
		$this->load->vars($data);
        $this->load->view("ajax_get_formulary_info");
    ...
    function autocomplete_diagnosis() // http://www.jamipietila.fi/codeigniter-and-autocomplete-with-jquery/
    {
        ...
        $rows = $this->majax_rdb->get_autocomplete_diagnosis($term);
