...
/**
 * Controller Class for Patient Records
 *
...
class Indv extends MX_Controller {
	...
	public function index()
	{
        ...
        $data['auto_refresh']   =    "";
        if($data['module_method'] == "individual_overview"){
            $data['auto_refresh']   =    "onload='timed_refresh(".$refresh_ms.");'";
        }
        
        $data['individual_info']   =    array();
		$data['individual_info']   = $this->memr_rdb->get_patient_demo($data['patient_id']);
        if ($data['patient_id'] == "new_patient") {
            // New patient
            $data['individual_info']['name']       =   " ";
            $data['individual_info']['name_first'] =   " ";
            $data['individual_info']['gender']     =   " ";
            $data['individual_info']['birth_date'] =   " ";
            $data['individual_info']['age_words']     =   " ";
        }
        $data['location_id']        =   $this->session->userdata('location_id');
		$data['clinic_info']        =   $this->mthirra->get_clinic_info($data['location_id']);
		$data['title']              =   "Record - ".$data['individual_info']['name'];
		$data['main']               =   'home';
		$data['acl_menu'] = $this->mthirra->get_user_acl($data['user_rights']['category_id'],'ovv','menu');
		$data['acl_panel'] = $this->mthirra->get_user_acl($data['user_rights']['category_id'],'ovv','panel');
		$data['acl_widget'] = $this->mthirra->get_user_acl($data['user_rights']['category_id'],'ovv','widget');
		$this->load->vars($data);
        
        // Determine VIEW components
        switch ($data['patient_id']){
            case "new_patient":
                $data['new_sidebar']    =   "nolink";
                break;			
            default:
                $data['new_sidebar']    =   "show";
        } //end switch ($data['seg3'])
        switch ($data['seg3']){
            case "new_draft":
		...
		if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            } //endif($data['seg3'] == "print_hardcopy")
            if($data['new_sidebar'] == "nolink"){
                $new_sidebar=   "ehr/sidebar_ehr_patients_ovrvwNoLink_wap";
            } else {
            ...
            if($data['new_sidebar'] == "nolink"){
                $new_sidebar=   "ehr/sidebar_ehr_patients_ovrvwNoLink_html";
            } else {
                $new_sidebar=   "ehr/sidebar_ehr_patients_ovrvw_html";
            } //endif($data['patient_id'] == "new_patient")            */
            $new_body   =   "indv_html";
            $new_footer =   "ehr/footer_ehr_html";
		} // endif ($this->session->userdata('thirra_mode') == "ehr_mobile")
