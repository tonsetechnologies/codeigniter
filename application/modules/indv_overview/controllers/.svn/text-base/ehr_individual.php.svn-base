 ...
 */
class Ehr_individual extends MX_Controller 
{
     ...
     * @author  Jason Tan Boon Teck
     */
	function individual_overview($seg3,$seg4)
    {
        ...
        $this->load->model('mprescribe_rdb');
        ...
        $data['vitals_info']    = $this->mindiv_rdb->get_recent_vitals($data['patient_id']);
        $data['problem_list']   = $this->mindiv_rdb->get_problem_list($data['patient_id']);
        $data['medication_info']= $this->mindiv_rdb->get_recent_medication($data['patient_id'],5,0);
        $data['procedure_info'] = $this->morders_procedure_rdb->get_past_procedures($data['patient_id']);
        $data['procedure_orders'] = $this->morders_procedure_rdb->get_recent_procedure_orders($data['patient_id']);
        $data['lab_info']       = $this->mindiv_rdb->get_recent_lab($data['patient_id']);
        $data['imaging_info']   = $this->mindiv_rdb->get_recent_imaging($data['patient_id']);
        $data['prediagnoses_info']= $this->mindiv_rdb->get_recent_diagnoses($data['patient_id'],TRUE);
        $data['diagnoses_info'] = $this->mindiv_rdb->get_recent_diagnoses($data['patient_id'],FALSE);
		$data['social_history'] = $this->mindiv_rdb->get_history_social('List',$data['patient_id']);
        $data['vaccines_list'] 	= $this->mprescribe_rdb->get_patient_immunisation($data['patient_id'],6,0);
        ...
        $data['referrals_list'] = $this->mrefer_rdb->get_history_referrals('Consulted','List',$data['patient_id']);
		...
		*/
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_body   =   "ehr_indv_overview_html";
    ...
    // ------------------------------------------------------------------------
    function edit_patient($seg3,$seg4,$seg5)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		...
		$data['form_purpose']       = $seg3; //$this->uri->segment(3);
		$data['clinic_info']        = $this->mthirra->get_clinic_info($data['location_id']);
        ...
        if(count($_POST)) {
            ...
            if ($data['form_purpose'] == "new_patient") {
                ...
                $data['init_birth_date']            =   $seg5; //$this->uri->segment(5);
		...
		$data['family_above']  = $this->mfamily_rdb->get_family_relations('List','above',$data['patient_id']);
		$data['family_below']  = $this->mfamily_rdb->get_family_relations('List','below',$data['patient_id']);
		...
		if ($this->form_validation->run($validate_criteria) == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_body   =   "ehr_edit_patient_demog_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
            ...
            if($data['patient_id'] == "new_patient") {
                ...
                $ins_patient_array['staff_id']           = $this->session->userdata('staff_id');
                ...
                $ins_patient_array['birth_date_estimate']= $data['init_birth_date_estimate'];
                ...
                $upd_patient_array['staff_id']           = $this->session->userdata('staff_id');
                ...
                $upd_patient_array['update_by']          = $this->session->userdata('staff_id');
                if($data['offline_mode']){
            ...
            echo form_open('indv/indv/index/indv_overview/ehr_individual/individual_overview/overview/'.$upd_patient_array['patient_id']);
    ...
    // ------------------------------------------------------------------------
    function hardcopy_patient_booklet($seg3,$seg4)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        $data['app_country']		=	$this->config->item('app_country');
		...
		if ($this->form_validation->run('edit_episode') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_body   =   "ehr_hardcopy_patient_booklet_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
    ...
    // ------------------------------------------------------------------------
    function past_con_details($seg3, $seg4, $seg5, $seg6)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        ...
        */
        ...
        $data['output_format'] 	= $seg6;//$this->uri->segment(5);
		$this->load->vars($data);
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_body   =   "ehr_reports_consult_details_html";
            $new_footer =   "ehr/footer_emr_wap";
        } else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
...
//$mpdf->WriteHTML('Your Book text');
    ...
    // ------------------------------------------------------------------------
    function list_family_cluster($seg3, $seg4)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
        ...
        if($data['family_link'] == "Head of Family"){
            ...
            $data['family_cluster']  = $this->mfamily_rdb->get_family_relations('List','below',$data['patient_id']);
        } elseif($data['family_link'] == "Under Head of Family") {
            //echo "Not Head";
            // Find out who is the head
            $data['head_info']  = $this->mfamily_rdb->get_family_head($data['patient_id']);
            ...
            $data['family_cluster']  = $this->mfamily_rdb->get_family_relations('List','above',$data['patient_id']);
        } else {
            //echo "Independent";
            // Find out who is the head
            $data['head_info']  = $this->mfamily_rdb->get_family_head($data['patient_id']);
        ...
        $data['family_cluster']  = $this->mfamily_rdb->get_family_relations('List','below',$data['family_head']);
		$data['family_above']  = $this->mfamily_rdb->get_family_relations('List','above',$data['patient_id']);
		$data['family_below']  = $this->mfamily_rdb->get_family_relations('List','below',$data['patient_id']);
		$this->load->vars($data);
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_banner =   "ehr/banner_ehr_ovrvw_wap";
            $new_sidebar=   "ehr/sidebar_ehr_patients_ovrvw_wap";
            $new_body   =   "ehr_indv_list_family_cluster_html";
            $new_footer =   "ehr/footer_emr_wap";
		} else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
    ...
    // ------------------------------------------------------------------------
    function edit_relationship_info($seg3, $seg4, $seg5) 
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
		$data['form_purpose']       =   $seg3; //$this->uri->segment(3);
		$data['patient_id']         =   $seg4; //$this->uri->segment(4);
		$data['relationship_id']    =   $seg5; //$this->uri->segment(5);
        ...
        $data['now_date']           =   date("Y-m-d",$data['now_id']);
        $data['now_time']           =   date("H:i",$data['now_id']);
		$data['heads_list']  = $this->mfamily_rdb->get_patientby_family_type('Head of Family');
        if($data['form_purpose'] == "new_relation"){
            $data['independents_list']  = $this->mfamily_rdb->get_patientby_family_type('Independent');
        } else {
            $data['independents_list']  = $this->mfamily_rdb->get_patientby_family_type('Under Head of Family',$data['patient_id']);
        }
        ...
        if(count($_POST)) {
            ...
            if ($data['form_purpose'] == "new_relation") {
                ...
                // Existing user
                $data['relationship_info']      = $this->mfamily_rdb->get_relationship_info($data['patient_id'],$data['relationship_id']);
		...
		if ($this->form_validation->run('edit_relationship_info') == FALSE){
            ...
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_sidebar=   "ehr/sidebar_ehr_patients_ovrvw_wap";
                $new_body   =   "ehr_indv_edit_relationship_info_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
            ...
            if($data['form_purpose'] == "new_relation") {
                ...
                $ins_relation_array['staff_id']         = $this->session->userdata('staff_id');
            ...
            $new_page = base_url()."index.php/indv/indv/index/indv_overview/ehr_individual/list_family_cluster/list_cluster/".$data['patient_id'];
            header("Status: 200");
            header("Location: ".$new_page);
    ...
    // ------------------------------------------------------------------------
    function change_family_link_type($seg3, $seg4, $seg5) 
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        $this->load->model('mehr_wdb');
        ...
        $data['form_purpose']       =   $seg3; //$this->uri->segment(3);
        $data['patient_id']         =   $seg4; //$this->uri->segment(4);
        $data['relationship_type']  =   $seg5; //$this->uri->segment(5);
    ...
    // ------------------------------------------------------------------------
    function delete_family_relationship($seg3, $seg4, $seg5) 
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        $this->load->model('mehr_wdb');
    ...
    // ------------------------------------------------------------------------
    function list_family_relations($seg3, $seg4)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_banner =   "ehr/banner_ehr_ovrvw_wap";
            $new_sidebar=   "ehr/sidebar_ehr_patients_ovrvw_wap";
            $new_body   =   "ehr_indv_list_family_relations_html";
            $new_footer =   "ehr/footer_emr_wap";
		} else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
    ...
    // ------------------------------------------------------------------------
    function list_drug_allergies($seg3, $seg4)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_banner =   "ehr/banner_ehr_ovrvw_wap";
            $new_sidebar=   "ehr/sidebar_ehr_patients_ovrvw_wap";
            $new_body   =   "ehr_indv_list_drug_allergies_html";
            $new_footer =   "ehr/footer_emr_wap";
		} else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
    ...
    function edit_drug_allergy($seg3, $seg4, $seg5)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
        ...
        if(count($_POST)) {
            ...
            $data['form_purpose']       = $seg3; //$this->uri->segment(3);
            $data['patient_id']         = $seg4; //$this->uri->segment(4);
            $data['patient_drug_allergy_id']     = $seg5; //$this->uri->segment(5);
        ...
        $data['init_location_id']   =   $this->session->userdata('location_id'); //$_SESSION['location_id'];
        $data['init_clinic_name']   =   NULL;
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_sidebar=   "ehr/sidebar_ehr_patients_ovrvw_wap";
                $new_body   =   "ehr_indv_edit_drug_allergy_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
            ...
            //echo "<br />Insert record";
                ...
                $ins_allergy_array['added_staff']       = $this->session->userdata('staff_id'); //$this->session->userdata('staff_id');
               ...
               if($data['offline_mode']){
                ...
                $this->session->set_flashdata('data_activity', 'Drug allergy added.');
            $new_page = base_url()."index.php/indv/indv/index/indv_overview/ehr_individual/edit_drug_allergy/new_allergy/".$data['patient_id']."/".$data['summary_id'];
            header("Status: 200");
            header("Location: ".$new_page);
    ...
    // ------------------------------------------------------------------------
    function edithist_immune_select($seg3, $seg4, $seg5)  // patient immunisation
	{
		...
		$data['patient_info']   = $this->memr_rdb->get_patient_demo($data['patient_id']);
        //$data['patcon_info']    = $this->memr_rdb->get_patcon_details($data['patient_id']);
        $data['vaccines_list'] 	= $this->mprescribe_rdb->get_vaccines_list($data['patient_id'],999,0);
		...
		if ($this->form_validation->run('edit_history_immune') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
				...
				$new_body   =   "ehr_edithist_immune_select_html";
				$new_footer =   "ehr/footer_emr_wap";
			} else {
				//$new_header =   "ehr/header_xhtml1-strict";
				$new_header =   "ehr/header_xhtml1-transitional";
            ...
            if($data['form_purpose'] == "new_immune") {
                ...
                $ins_prescribe_array['staff_id']         = $this->session->userdata('staff_id'); //$this->session->userdata('staff_id');
                ...
                $upd_prescribe_array['staff_id']        = $this->session->userdata('staff_id'); //$this->session->userdata('staff_id');
            ...
            $new_page = base_url()."index.php/indv/indv/index/indv_overview/ehr_individual/edithist_immune_select/new_immune/".$data['patient_id']."/new_immune";
            header("Status: 200");
            header("Location: ".$new_page);
    ...
    // ------------------------------------------------------------------------
    function histdel_immune_confirm($seg3, $seg4, $seg5, $seg6)  // patient immunisation
	{
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        $this->load->model('mprescribe_rdb');
        ...
        $data['patient_id']             = $seg4; //$this->uri->segment(3);
        $data['patient_immunisation_id']= $seg5; //$this->uri->segment(4);
        $data['prescribe_id']           = $seg6; //$this->uri->segment(5);
			
		$data['patient_info']   = $this->memr_rdb->get_patient_demo($data['patient_id']);
        //$data['patcon_info']    = $this->memr_rdb->get_patcon_details($data['patient_id']);
        $data['vaccines_list'] 	= $this->mprescribe_rdb->get_patient_immunisation($data['patient_id'], 0);
		// This array can be refactored to be automatic
		$data['vaccination_table']	= array(0,1,2,3,4,5,6,12,18,24);		
        //$data['prescribe_list'] = $this->memr_rdb->get_patcon_prescribe($data['summary_id']);
        $data['init_location_id']   =   $this->session->userdata('location_id'); //$_SESSION['location_id'];
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_body   =   "ehr_histdel_immune_confirm_html";
            $new_footer =   "ehr/footer_emr_wap";
        } else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
    ...
    function histdel_immune_exec($id=NULL) 
    {
        ...
        $new_page = base_url()."index.php/indv/indv/index/indv_overview/ehr_individual/edithist_immune_select/new_immune/".$data['patient_id'];
        header("Status: 200");
        header("Location: ".$new_page);
    ...
    // ------------------------------------------------------------------------
    function upload_pics_ovv($seg3, $seg4, $seg5)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
        ...
        */
	    ...
	    if ( ! $this->upload->do_upload())
	    {
            ...
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_banner =   "ehr/banner_ehr_ovrvw_wap";
                $new_sidebar=   "ehr/sidebar_ehr_patients_ovrvw_wap";
                $new_body   =   "ehr_patients_upload_ovv_html";
                $new_footer =   "ehr/footer_emr_wap";
		    } else {
                $new_header =   "ehr/header_xhtml1-transitional";
                $new_banner =   "ehr/banner_ehr_ovrvw_html";
            ...
            */
            ...
            //}
            $ins_pics_array['staff_id']         =   $this->session->userdata('staff_id');//$this->session->userdata('staff_id');
            ...
            $ins_pics_array['location_id']      =   $this->session->userdata('location_id');//$_SESSION['location_id'];
            ...
            header("Status: 200");
            redirect('indv/indv/index/indv_overview/ehr_individual/upload_pics_ovv/new_file/'.$data['patient_id'],'refresh');
            /*
                echo "<pre>";
                print_r($upload_data);
                $this->load->view('test_upload', $error);
    ...
    // ------------------------------------------------------------------------
    function ovv_externalmod($seg3, $seg4)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
		...
		$data['staff_id']       = $this->session->userdata('staff_id');//$this->session->userdata('staff_id');
        $data['patient_id']     = $seg4; //$this->uri->segment(4);
        //$data['summary_id']     = $this->uri->segment(5);
		//$data['clinic_info']    = $this->mbio->get_clinic_info($_SESSION['location_id']);
		$data['patient_info'] = $this->memr_rdb->get_patient_demo($data['patient_id']);
 		$data['title'] = "PR-".$data['patient_info']['name'];
		$data['broken_birth_date'] =   break_date($data['patient_id']['birth_date']);
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_banner =   "ehr/banner_ehr_ovrvw_wap";
            $new_sidebar=   "ehr/sidebar_ehr_patients_ovrvw_wap";
			$new_body   =   "ehr_indv_ovv_externalmod_html";
    ...
    // ------------------------------------------------------------------------
    function graph_processor($id=NULL)  // DEPRECATED
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
