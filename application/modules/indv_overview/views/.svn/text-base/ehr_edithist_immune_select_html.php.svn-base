...
if($debug_mode) {
    ...
    print "<br />summary_id = " . $summary_id;
	echo '<pre>';
...
echo form_open('indv/indv/index/indv_overview/ehr_individual/edithist_immune_select/new_immune/'.$patient_id.'/'.$summary_id, $attributes);
        ...
        if($current_column > $previous_column){
            if($vaccine['age_group'] == $age_column){
                ...
                if(isset($vaccine['date'])){
                    //if($summary_id == $vaccine['session_id']){
                        echo "(";
                	    echo anchor('indv/indv/index/indv_overview/ehr_individual/histdel_immune_confirm/delete_immune/'.$patient_id.'/'.trim($vaccine['patient_immunisation_id']), 'x');
                        //echo "(x)";
                        echo ")";
                    //}
                }
...
echo "\n<input type='text' name='vaccine_date' id='vaccine_datepicker' value='".set_value('vaccine_date',$vaccine_date)."' size='8' /> YYYY-MM-DD</td></tr>";
