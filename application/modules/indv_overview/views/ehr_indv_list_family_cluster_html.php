...
if(($patient_info['family_link'] == "Head of Family") || !empty($family_cluster)){
    ...
    echo anchor('indv/indv/index/indv_overview/ehr_individual/change_family_link_type/change_type/'.$patient_id.'/head', "<strong>Head of Family</strong>");
}
echo "<br />\n";
if(($patient_info['family_link'] == "Independent") || !empty($family_cluster)){
    echo "Independent";
} else {
    echo anchor('indv/indv/index/indv_overview/ehr_individual/change_family_link_type/change_type/'.$patient_id.'/independent', "<strong>Independent</strong>");
}
echo "<br />\n";
if(($patient_info['family_link'] == "Under Head of Family") || !empty($family_cluster)){
    echo "Under Head of Family";
} else {
    echo anchor('indv/indv/index/indv_overview/ehr_individual/change_family_link_type/change_type/'.$patient_id.'/under', "<strong>Under Head of Family</strong>");
}
echo "\n<p><br />";
...
if(count($head_info) > 0){
    if($head_info[0]['patient_id'] == $patient_id){
        ...
        $overview_sheet =   anchor('indv/indv/index/indv_overview/ehr_individual/individual_overview/overview/'.$head_info[0]['patient_id'], "<strong>".$head_info[0]['name']."</strong>, ".$head_info[0]['name_first'], 'target="_blank"');
...
if((count($head_info) < 1) && ($patient_info['family_link'] == "Under Head of Family")){
    echo anchor('indv/indv/index/indv_overview/ehr_individual/edit_relationship_info/new_relation/'.$patient_id.'/new_relation', "<strong>Add New Relationship</strong>");
}
...
if(isset($family_cluster)){
    ...
    foreach ($family_cluster as $cluster_item){
        if($cluster_item['patient_id'] == $patient_id){
            ...
            $which_window   =   "";
            $delete_link    =   anchor('indv/indv/index/indv_overview/ehr_individual/delete_family_relationship/delete_relation/'.$cluster_item['patient_id'].'/'.$cluster_item['relationship_id'], "Delete", $which_window);
        } else {
            $overview_sheet =   anchor('indv/indv/index/indv_overview/ehr_individual/individual_overview/overview/'.$cluster_item['patient_id'], "<strong>".$cluster_item['name']."</strong>, ".$cluster_item['name_first'], 'target="_blank"');
            $which_window   =   'target="_blank"';
            $delete_link    =   "";
        } //endif($cluster_item['patient_id'] == $patient_id)
        ...
        if(isset($cluster_item['family_position'])){
            echo "\n<td>".anchor('indv/indv/index/indv_overview/ehr_individual/edit_relationship_info/edit_relation/'.$cluster_item['patient_id'].'/'.$cluster_item['relationship_id'], "<strong>".$cluster_item['family_position']."</strong>", $which_window)."</td>";
        } else {
            echo "\n<td>Define</td>";
...
echo "\n<p>";
echo anchor('indv/indv/index/indv_overview/ehr_individual/list_family_relations/list_relations/'.$patient_id, 'Family Chain');
echo "</p>";
