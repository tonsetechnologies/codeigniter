...
if(isset($queue_list)){
    ...
    foreach ($queue_list as $queue_item){
        ...
        if($queue_item['clinic_shortname'] == $this->session->userdata('clinic_shortname')){
            echo anchor('indv/indv/index/indv_overview/ehr_individual_schedule/indv_edit_queue/edit_queue/'.$patient_id.'/'.$queue_item['booking_id'], $queue_item['date']." &nbsp;&nbsp;&nbsp; ".substr($queue_item['start_time'],0,5));
        } else {
            echo $queue_item['date']." &nbsp;&nbsp;&nbsp; ".substr($queue_item['start_time'],0,5);
        }
        echo "<t/d>";
        //echo "\n<td>".anchor('indv/indv/index/indv_overview/ehr_individual_schedule/indv_edit_queue/edit_queue/'.$patient_id.'/'.$queue_item['booking_id'], $queue_item['date']." &nbsp;&nbsp;&nbsp; ".$queue_item['start_time']."</td>");
...
echo "\n<h2>".$save_attempt."</h2>";
echo form_open('indv/indv/index/indv_overview/ehr_individual_schedule/indv_edit_queue/'.$form_purpose.'/'.$patient_id);
echo "\n<table>";
echo "<tr><td>";
echo "\n<table>"; // left column
