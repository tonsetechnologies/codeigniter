...
if(count($diagnosis_list) > 0){
    foreach ($diagnosis_list as $diagnosis_item){
        echo "\n<tr>";
        echo "<td>".anchor('cons/cons/index/cons_diagnosis/ehr_consult_diagnosis/edit_diagnosis/edit_diagnosis/'.$patient_id.'/'.$summary_id.'/'.$diagnosis_item['diagnosis_id'], $diagnosis_item['dcode1ext_code'])."</td>";
        ...
        $delete_link    =   base_url()."index.php/cons/cons/index/cons_diagnosis/ehr_consult_diagnosis/consult_delete_diagnosis/delete_diagnosis/".$patient_id."/".$summary_id."/".$diagnosis_item['diagnosis_id'];
        echo "\n<td><a href=\"javascript:deleteRecord('remove','Diagnosis:".$diagnosis_item['full_title']."','".$delete_link ."');\">delete</a>";
        //echo "\n<td>".anchor('cons/cons/index/cons_diagnosis/ehr_consult_diagnosis/consult_delete_diagnosis/'.$patient_id.'/'.$summary_id.'/'.$diagnosis_item['diagnosis_id'], 'delete')."</td>";
        echo "</tr>";
    }//endforeach;
} else {
...
echo "\n\n<div id='patients_new_diagnosis' class='section'>";
    ...
    echo form_open('cons/cons/index/cons_diagnosis/ehr_consult_diagnosis/edit_diagnoses/'.$form_purpose.'/'.$patient_id.'/'.$summary_id.'/'.$diagnosis_id, $attributes);
    ...
    echo "\n<br /><input type='text' name='diagnosis_term1' id='diagnosis_term1' value='' size='100' />";
...
echo "\n<tr>";
    ...
    echo "\n<td>";
        echo "<select class='normal' name='diagnosis_type'>";
            ...
            echo "\n<option value='Provisional' ".set_select('diagnosis_type','Provisional').">Provisional</option>";
        echo "</select>";
...
echo "<br />";
echo "<center>";
    echo "<input class='button' type='submit' name='submit' value='Add/Edit Diagnosis' />";
echo "</center>";
...
//	 $.ajax({ url: "/thirra/index.php/cons/cons/index/cons_diagnosis/ehr_consult_diagnosis/autocomplete_diagnosis",
// Set for AJAX calling
$siteurl_diagnosiscategory    =   site_url()."/ehr_ajax/autocomplete_diagnosis";
?>
<br />
...
<script  type="text/javascript">
    ...
    $(document).ready(function() {
	    ...
	    $(function() {
		    $( "#diagnosis_term1" ).autocomplete({
			    source: function(request, response) {
				    $.ajax({ url: siteurl,
				    data: { term: $("#diagnosis_term1").val()},
				    dataType: "json",
				    type: "POST",
