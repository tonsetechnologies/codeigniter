 ...
 */
class Ehr_consult_diagnosis extends MX_Controller 
{
    ...
    function __construct()
    {
		...
		$this->load->model('mdiagnosis_rdb');
    ...
    function edit_diagnosis($seg3,$seg4,$seg5,$seg6)
    {
        ...
        if(count($_POST)) {
            ...
            $data['diagnosis_id']   =   $seg6; //$this->uri->segment(6);
		...
		if ($this->form_validation->run('edit_diagnosis') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_body   =   "ehr_edit_diagnosis_html";
            ...
            if($data['form_purpose'] == "new_diagnosis") {
                ...
                $ins_diagnosis_array['staff_id']           = $this->session->userdata('staff_id');
    ...
    function edit_diagnoses($seg3,$seg4,$seg5,$seg6)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        ...
        if(count($_POST)) {
            ...
            $data['diagnosis_notes'] = $_POST['diagnosis_notes'];
            if(isset($data['diagnosis'])){
                $data['diagnosis_chosen'] = $this->mdiagnosis_rdb->get_one_diagnosis_code($data['diagnosis']);
            } //endif(isset($data['diagnosis']))
            if(($data['form_id'] == "search") && (strlen($data['diagnosis_term1'])>2)){
                $data['diagnosis_filter'] = $this->mdiagnosis_rdb->get_diagnosis_list($data['diagnosis_pullall'],$data['diagnosis_term1']);
        ...
        $data['patcon_info']    = $this->memr_rdb->get_patcon_details($data['patient_id']);
        $data['diagnosis_list'] = $this->memr_rdb->get_patcon_diagnosis($data['summary_id']);
        $data['diagnosis_common'] = $this->mdiagnosis_rdb->get_common_diagnosis();
        $data['init_location_id']   =   $this->session->userdata('location_id');
        $data['init_clinic_name']   =   NULL;
        $data['now_id']             =   time();
        $data['now_date']           =   date("Y-m-d",$data['now_id']);
		...
		if ($this->form_validation->run('edit_diagnosis') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_body   =   "ehr_edit_diagnoses_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
    ...
    // ------------------------------------------------------------------------
    function consult_delete_diagnosis($seg3,$seg4,$seg5,$seg6) 
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        $data['form_purpose']   =   $seg3; //$this->uri->segment(3);
    ...
    // ------------------------------------------------------------------------
    // Categorised diagnosis form
    function edit_prediagnosis($seg3,$seg4,$seg5,$seg6)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$this->load->model('mutil_rdb');
		...
		if ($this->form_validation->run('edit_diagnosis') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_sidebar=   "ehr/sidebar_ehr_patients_conslt_wap";
                //$new_body   =   "ehr/emr_edit_diagnosis_wap";
                $new_body   =   "ehr_edit_prediagnosis_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
