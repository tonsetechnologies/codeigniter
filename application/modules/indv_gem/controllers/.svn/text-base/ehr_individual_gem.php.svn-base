 ...
 */
class Ehr_individual_gem extends MX_Controller 
{
    ...
    // === INDIVIDUAL RECORD
    // ------------------------------------------------------------------------
    function list_gem_submodules($seg3,$seg4,$seg5)  
    {
        ...
        $data['gem_module_id']      =   $seg5; //$this->uri->segment(5);
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_body   =   "ehr_indv_list_gem_submodules_html";
    ...
    // ------------------------------------------------------------------------
    function list_history_gemsubmodule($seg3,$seg4,$seg5,$seg6)  
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_sidebar=   "ehr/sidebar_ehr_patients_ovrvw_wap";
            //$new_body   =   "ehr/ehr_edit_immune_select_wap";
            $new_body   =   "ehr_indv_list_history_gemsubmodule_html";
            $new_footer =   "ehr/footer_emr_wap";
		} else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
    ...
    // ------------------------------------------------------------------------
    function list_history_gemsessions($seg3,$seg4,$seg5,$seg6)  // List drug packages
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
		$data['user_rights']        =   $this->mthirra->get_user_rights($this->session->userdata('username'));
        ...
        if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
            ...
            $new_sidebar=   "ehr/sidebar_ehr_patients_ovrvw_wap";
            //$new_body   =   "ehr/ehr_edit_immune_select_wap";
            $new_body   =   "ehr_indv_list_history_gemsessions_html";
            $new_footer =   "ehr/footer_emr_wap";
		} else {
            //$new_header =   "ehr/header_xhtml1-strict";
            $new_header =   "ehr/header_xhtml1-transitional";
    ...
    // ------------------------------------------------------------------------
    // GEM submodule form
    function gem_view_consult($seg3,$seg4,$seg5,$seg6,$seg7)
    {
        $data['offline_mode']		=	$this->config->item('offline_mode');
        $data['debug_mode']		    =	$this->config->item('debug_mode');
        $data['form_purpose']       =   $seg3; //$this->uri->segment(3);
        ...
        $data['gem_module_id']      =   $seg5; //$this->uri->segment(5);
        $data['gem_submod_id']      =   $seg6; //$this->uri->segment(6);
        $data['gem_session_id']     =   $seg7; //$this->uri->segment(7);
		$data['title'] = "GEM";
		$data['patient_info']       = $this->memr_rdb->get_patient_demo($data['patient_id']);
        $data['now_id']             =   time();
        ...
        */
		...
		if ($this->form_validation->run('edit_consult_gem') == FALSE){
            if ($this->session->userdata('thirra_mode') == "ehr_mobile"){
                ...
                $new_banner =   "ehr/banner_ehr_ovrvw_wap";
                $new_sidebar=   "ehr/sidebar_ehr_patients_ovrvw_wap";
                $new_body   =   "ehr_indv_gem_view_consult_html";
                $new_footer =   "ehr/footer_emr_wap";
            } else {
                //$new_header =   "ehr/header_xhtml1-strict";
                $new_header =   "ehr/header_xhtml1-transitional";
