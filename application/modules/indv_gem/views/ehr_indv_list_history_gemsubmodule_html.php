...
echo "\n<tr>";
    ...
    echo "<td>:</td>";
    echo "<td>";
    echo anchor('indv/indv/index/indv_gem/ehr_individual_gem/list_gem_submodules/'.$submodule_info[0]['gem_module_id'].'/'.$patient_id, $submodule_info[0]['gem_module_name']);
    echo "</td>";
echo "</tr>";
echo "\n<tr>";
...
if(isset($sessions_list)){
    ...
    foreach ($sessions_list as $session_item){
        ...
        if(!empty($session_item['date_ended'])){
            echo anchor('indv/indv/index/indv_gem/ehr_individual_gem/gem_view_consult/view_consult/'.$patient_id.'/'.$session_item['gem_module_id'].'/'.$session_item['gem_submod_id'].'/'.$session_item['gem_session_id'], $session_item['date_ended']."&nbsp;&nbsp;&nbsp;".$session_item['time_ended']);
        } else {
            echo "In progress";
        }
        echo "</td>";
