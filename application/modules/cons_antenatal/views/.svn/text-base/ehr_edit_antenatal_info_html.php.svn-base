...
echo form_open('cons/cons/index/cons_antenatal/ehr_consult_antenatal/edit_antenatal_info/'.$form_purpose.'/'.$patient_id.'/'.$summary_id, $attributes);
...
echo "\n<tr>";
    ...
    if(!empty($init_contact_person)) {
        echo "\n&nbsp;&nbsp;";
        echo anchor('indv/indv/index/indv_overview/ehr_individual/individual_overview/overview/'.$init_contact_person, "<strong>View kin</strong>", 'target="_blank"');
    }
    echo "\n</td>";
echo "\n</tr>";
...
if($save_attempt == "EDIT ANTENATAL INFO"){
    ...
    if(isset($followup_list)){
        ...
        foreach ($followup_list as $followup_item){
            ...
            echo "\n<td>".$rownum.".</td>";
            echo "\n<td>";
            echo ($followup_item['session_id']==$summary_id? "<strong>" : "");
            echo anchor('cons/cons/index/cons_antenatal/ehr_consult_antenatal/edit_antenatal_followup/edit_followup/'.$patient_id.'/'.$summary_id.'/'.$antenatal_id.'/'.$followup_item['antenatal_followup_id'], $followup_item['date']);
            echo ($followup_item['session_id']==$summary_id? "</strong>" : "");
            echo "</td>";
    ...
    echo anchor('cons/cons/index/cons_antenatal/ehr_consult_antenatal/edit_antenatal_delivery/new_delivery/'.$patient_id.'/'.$summary_id.'/'.$antenatal_id.'/new_delivery', "<strong>Add New Delivery</strong>");
    echo "\n<br /><br />";
    ...
    if(isset($delivery_list)){
        ...
        foreach ($delivery_list as $delivery_item){
            echo "\n<tr>";
            echo "\n<td>".$rownum.".</td>";
            echo "\n<td>".anchor('cons/cons/index/cons_antenatal/ehr_consult_antenatal/edit_antenatal_delivery/edit_delivery/'.$patient_id.'/'.$summary_id.'/'.$antenatal_id.'/'.$delivery_item['antenatal_delivery_id'], "<strong>".$delivery_item['date_delivery']."</strong>")."</td>";
    ...
    echo "\n<tr>";
        ...
        echo "\n<th width='50'>Vaginal discharge</th>";
        echo "\n<th width='50'>Vaginal bleeding</th>";
        echo "\n<th >Pallor</th>";
        echo "\n<th width='50'>Remarks</th>";
    echo "</tr>";
    $current_postpartum_exists =   FALSE;
    if(isset($postpartum_list)){
        ...
        foreach ($postpartum_list as $postpartum_item){
            ...
            echo anchor('cons/cons/index/cons_antenatal/ehr_consult_antenatal/edit_antenatal_postpartum/edit_postpartum/'.$patient_id.'/'.$summary_id.'/'.$antenatal_id.'/'.$postpartum_item['antenatal_postpartum_id'], $postpartum_item['care_date']);
            echo ($postpartum_item['session_id']==$summary_id? "</strong>" : "");
            echo "</td>";
            ...
            echo "\n<td>".$postpartum_item['vaginal_discharge']."</td>";
            echo "\n<td>".$postpartum_item['vaginal_bleeding']."</td>";
            echo "\n<td>".$postpartum_item['pallor']."</td>";
            echo "\n<td>".$postpartum_item['postpartum_remarks']."</td>";
            echo "\n</tr>";
            if($postpartum_item['session_id'] == $summary_id){
                $current_postpartum_exists =   TRUE;
            }
            $rownum++;
        }//endforeach;
    ...
    echo "</table>";
    echo "\n<br /><br />";
    if(count($delivery_list) > 0 && $current_postpartum_exists === FALSE){
        echo anchor('cons/cons/index/cons_antenatal/ehr_consult_antenatal/edit_antenatal_postpartum/new_postpartum/'.$patient_id.'/'.$summary_id.'/'.$antenatal_id.'/new_postpartum', "<strong>Add New Postpartum Care</strong>");
    //} else {
